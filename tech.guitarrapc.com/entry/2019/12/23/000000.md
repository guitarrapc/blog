...
* Pulumi は、デフォルト挙動では Pulumi State と 実リソースの差を同期しない
* Terraform はデフォルトで、State と実リソースの差を同期する
* Pulumi で、Terraform 同様にState と実リソースの差分を同期するには `pulumi refresh` や `pulumi up --refresh` を使う
