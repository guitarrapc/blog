...
.NET SDK 9.0.200からSLNXと呼ばれる新ソリューションファイルフォーマットが利用しやすくなったので、本記事で触っていきます。
...
C#のプロジェクト構成は3パターンに大別できます。[^1]SLNX形式は2025年3月現在、プレビュー版として提供されています。
...
![image](https://github.com/user-attachments/assets/cdb622b9-3c59-4a21-9d92-6e5028c11a7b)


## プロジェクトファイルとソリューションファイル

ソリューションファイルは、複数のプロジェクトファイルを束ねるファイルです。Visual Studioで開くと、ソリューションエクスプローラーに登録したプロジェクトが表示されます。プロジェクトファイルは、プロジェクトの設定やビルド設定を記述します。両者を用いることで、C#プロジェクトは複数のプロジェクトの依存関係を管理し、ビルドやデバッグを効率的に行うことができます。

### プロジェクトファイル

C#プロジェクトファイルは拡張子`.csproj`、XMLでプロジェクトの設定やビルド設定を記述します。.NET Coreで[SDK-Style](https://learn.microsoft.com/en-us/dotnet/core/project-sdk/overview)と呼ばれるフォーマットが導入されて、.NET Frameworkのころに比べてシンプルかつ扱いやすくなりました。
...
![image](https://github.com/user-attachments/assets/317b6ecd-e8ad-43ee-b760-2c32ba61a68e)

| プロジェクト | 旧csproj | SDK Style csproj |
| --- | --- | --- |
| .NET Framework WebProject | ![image](https://github.com/user-attachments/assets/8edf76b7-6532-44ce-9fd6-df9377a7290f) | ![image](https://github.com/user-attachments/assets/82ac594d-5b09-4201-bfff-a9d5d95b3e43)


### ソリューションファイル

[ソリューションファイル](https://learn.microsoft.com/ja-jp/visualstudio/extensibility/internals/solution-dot-sln-file?view=vs-2022)は拡張子`.sln`で、独自形式で記載されているのですが、人間が直接書くことはないのであまり見る機会はないかもしません。.NET Coreになっても.slnが利用されてきましたが、2024年から新形式となるSLNX形式がプレビュー提供されています。とはいえ、SLNX形式はまだGAが見えていません。

ソリューションファイル`.sln`は次のように独自形式になっています。
...
.slnの特徴は、プロジェクトごとにGUIDが振られていることです。GUIDはソリューションにプロジェクトを追加すると勝手にふられます。直接いじる機会はないものの手動編集するには無駄に面倒でした。またファイルヘッダー部分でVisual Studioのバージョンも書かれています。結果として、.slnファイルはGit差分競合を起こしやすく、使っていて難しいと感じだ人も多いのではないでしょうか。

* プロジェクトGUID: プロジェクトの追加削除でGUIDが変わるため、元ブランチでcsprojを追加したところにPRでcsprojを追加されると競合が確定で発生する
* Visual Studioバージョン: 開いたVisual Studioのバージョンで勝手に書き換わるため、不用意に書き換わる可能性がある

せっかくcsprojがシンプルになったので同様にslnxもシンプルになってほしい、という[要望](https://github.com/dotnet/msbuild/issues/1730#issuecomment-281446717)は以前から出ていました。

# SLNX形式

SLNX形式は、次のようなフォーマットです。csprojがXMLなのも相まって割と人間が理解しやすい形式になっています。
...
.slnと比べて、.slnxは以下のような特徴でシンプルになっていることがわかります。コメントを残すこともでき、Git差分競合も起こりにくくなっています。

| 要素 | .sln | .slnx |
| --- | --- | --- |
| ヘッダーセクション | あり | なし |
| プロジェクト | GUIDで管理 | パスで管理 |
| プラットフォーム | `SolutionConfigurationPlatforms`で管理 | `<Configurations.Platform>`で管理 |
| プロジェクトごとのプラットフォーム| GUID+プラットフォーム文字列で管理 | プロジェクトで共通なら指定なし |
| コメントを入れられるか | 不可 | 空白、コメントが保持されるようになった |

## .NET SDK 9.0.200+からSLNX形式を実用的に試せるようになった

これまでもVisual StuioでSLNX形式を作ったり開くことはできたものの、dotnet buildコマンドが対応しておらず試す意味がありませんでした。しかし、.NET SDK 9.0.200で以下の2点が入り「SLNX形式は使いものになるのか」を試す土台が整いました。

* MSBuildがSLNX形式に対応
* dotnet cliがSLNX形式をサポート
* `dotnet sln migrate`コマンドで.slnから.slnxに変換

なお、`dotnet new slnx`コマンドはまだないため、既存のslnをslnxに変換することが前提となります。
...
.slnxファイルは、Visual Studio Selectorで認識していない拡張子ですが、Visual Studio 2022以降[^2]で扱うことができます。

![image](https://github.com/user-attachments/assets/cd40e670-d781-4ce0-8f14-bcfe7c312e0b)

Visual Studioで開くには、`Settings > Options > Environment > Preview Features > Use Solution File Persistence Model`オプションを有効にします。

![image](https://github.com/user-attachments/assets/71995549-b05c-404a-af09-24a2a311dba1)

オプションが有効になっていれば、`Visual Studio Selector`や`Visual Studio 2022`でSLNX形式のソリューションファイルを開くことができます。

![image](https://github.com/user-attachments/assets/59b8e959-d2da-48e4-8e1e-2b986abf2e8a)

![image](https://github.com/user-attachments/assets/3873bff1-1c1c-43ae-ba6a-1126231dfeed)

オプションが有効になっていない場合、.slnxを開くときにエラーが出ます。

![image](https://github.com/user-attachments/assets/5ce9a458-d4e8-4f78-8332-842c2e533d5e)

![image](https://github.com/user-attachments/assets/adeceb21-3469-4f42-8d95-46de40ffdadd)

![image](https://github.com/user-attachments/assets/36af050e-29c1-470a-be6b-9a76c2e56982)
...
![image](https://github.com/user-attachments/assets/c82738a8-885c-4db6-b0c8-4a22eb91354f)
...
Microsoftとしては、slnとslnxファイルを両方維持するには難しいので、slnxファイルだけにすることを推奨しています。例えばCIでソリューションファイルをターゲットにビルドする場合、.slnと.slnxが両方あるとビルドできません。
...
また、.slnと.slnxの両方を同期するのは人間がやるには厳しいため1つに絞るか、両方併用するなら[dotnet-sln-sync](https://github.com/edvilme/dotnet-sln-sync)[^1]の利用を推奨しています。
...
でも、csprojのパス参照でファイルグロブが使えるので、言ってることに一貫性がない気もします。個人的にはファイルグロブをサポートしたほうが圧倒的に管理しやすく、扱いやすくなると考えますが将来に注目です。[^3]

## GAはいつ?

不明です。早くGAできるように、フィードバックを送っていきましょう。
