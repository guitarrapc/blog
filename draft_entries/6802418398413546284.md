...
世の中にはHTMLをマークダウンに変換するツールがたくさんあるので、それをつかっても良かったですね。残念ながら記事分量が多すぎてLLMで変換もできなかったので、手作業で修正しました。私は`<a>`タグや`<pre>`タグの属性指定がそろってなかったことに起因した修正が多かったです。文中の`<code>`タグも割と抜けてました。
