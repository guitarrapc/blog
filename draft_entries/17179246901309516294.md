...
![C#のQRコードライブラリとの比較](image-2.png)<!--image-2.png-->

<details><summary>0.8.0のパフォーマンス</summary>

何も最適化していない状態です。

![0.8.0のパフォーマンス](image.png)<!--image.png-->

</details>

<details><summary>0.9.0のパフォーマンス</summary>

![0.9.0のパフォーマンス](image-1.png)<!--image-1.png-->
...
![LINQPadでQRコード表示](image-3.png)<!--image-3.png-->
...
![コンソールアプリケーションでemojiを使ってQRコード表示](image-4.png)<!--image-4.png-->
...
![Instagram風QRコード](image-5.png)<!--image-5.png-->
...
![classからstructへの変更](image-6.png)<!--image-6.png-->

LINQの線形探索をルックアップテーブルに変更

![LINQの線形探索をルックアップテーブルに変更](image-7.png)<!--image-7.png-->

QRCode表現をBitArrayから1次元バイナリに変更

![QRCode表現をBitArrayから1次元バイナリに変更](image-8.png)<!--image-8.png-->

マスク処理のリフレクションをswitchベースの分岐に変更

![マスク処理のリフレクションをswitchベースの分岐に変更](image-9.png)<!--image-9.png-->
...
![モジュールアクセスのペナルティ計算ループを可能な限り減らす](image-11.png)<!--image-11.png-->

モジュール配置をrectangleを用いたブロックチェックからbitmaskに変更

![モジュール配置をrectangleを用いたブロックチェックからbitmaskに変更](image-10.png)<!--image-10.png-->

事前計算できるmod/div演算をテーブル化して高速化

![事前計算できるmod/div演算をテーブル化して高速化](image-12.png)<!--image-12.png-->
...
ゼロアロケーションは何度も検討しましたが、使い勝手と両立する道が見えず見送っています。理屈としては利用者が`IBuffferWriter<byte>`を渡すAPIとすれば設計可能ですが、QuietZoneSizeを利用者が計算してレンダリングさせるのはちょっと厳しい印象です。今は1次元配列にしつつインデックスアクセスを提供することで、メモリ効率と使い勝手のバランスを取っています。将来的にゼロアロケーション版APIを追加する可能性はありますが、現状はアロケーションの小ささから見て優先度が低いと考えています。
