...
CustomPath: 2025/02/27/235900
Draft: true
---

C#9から利用できる[Target-typed new](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/proposals/csharp-9.0/target-typed-new)は使い方によっては非常に便利です。
ただ、Pulumi C#では使いたいときに使えないというシーンが多く出くわしたのでメモです。
...
このパターンはリソースごとにTResourceArgs型は違いますが、やってることは同じなので型の意識は重要じゃありません。そのため、Target-typed newを使うと設定に注目できます[^1]。いい感じですね。
...
さて、各リソースの引数`TResourceArgs`の各プロパティ`Input<T>`は定義されており、リソースの作成結果は`Output<T>`です。`Input<T>`は`Output<T>`の暗黙的型変換を持っており、これでリソース間の依存関係グラフを構築、実行制御します。
...
`Input<T>`は`T`に対する暗黙的型変換を持っているので、次のように直接Tを割り当ててシンプルに書けます。
...
`Input<T>`を使って定義することでリソースの依存関係を解決して、まだ存在しないリソースに対応している。これがPulumiにおけるリソースの作成の基本です。
...
Dictionary<string, List<int>>() = new() {
    { "item1", new() { 1, 2, 3 } }
};
```
...
* 設定系の型はあまりに冗長なケース (型の重要度が低く、設定はシンプルであるほど読みやすくなる)

## Target-typed newが使いにくいシーン

Target-typed newが出たときにも指摘がありましたが、Target-typed newはあまり読みやくないという指摘はちらちら見かけます。私も割と同意するところです。
...
インスタンス化するときにTarge-typed newを使うのも、限定的なシーン以外では読みにくくなっていると感じます。数年たっても、割と型指定したほうが読みやすいと感じます。
...
極端な話をあげると、連鎖的な依存を持つ複雑なコンストラクターでより読みにくくなると感じます。
...
コンストラクター引数で感じたことは、メソッド引数でも同様に限定的なシーン以外では読みにくくなっていると感じます。

```cs
// 引数の型が冗長という指摘もあるが、varでも型は自明
...
こういった、使い方に対する指針があまり定まっていないしコミュニティでの醸成はこれからなのかなという感じです。
