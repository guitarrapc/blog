...
IAM UserのAccessKeyやAccessSecretKeyの問題点は、一度発行したAccessKey/AccessSecretKeyに期限がいため認証漏洩時のインパクトが大きく、個人ごとに認証を発行する手間もありました。

AWS IAM Identity Centerの場合、`aws sso login`などのSSOログインで取得した一時認証情報に期限があります。また`aws sso login`をベースにすると、`.aws/credentials`や`.aws/config`にAWS AccessKey/AccessSecretKeyを記載不要なのもうれしいポイントでしょう。IAM UserのようにAccessKey/AccessSecretKeyを使う場合でも、AWSアクセスポータルで自動発行される上、時限があるのも良い点です。ただ、AccessKey/AccessSecretKeyを使うと`.aws/credentials`や環境変数に埋める必要があるため、`aws sso login`をベースにログインしたほうが使い勝手も良いです。

認証を利用する側、管理する側両方にとって以下の2点がメリットと言えます。

* 認証の時限化が強制される
* AccessKey/AccessSecretKeyを埋め込む必要がない

逆に面倒なポイントも裏返しとなります。C#アプリケーションでAWSのリソースにアクセスする時はどうすればいいのか見てみましょう。
...
NuGetパッケージは、IAM Userの時と違いAWSSDK.SSOとAWSSDK.SSOIDCの2パッケージが必要なことに注意してください。特に`AWSSDK.SSOOIDC`はなくてもコンパイルできるのですが、ないと実行時に無応答になるので必ず含めましょう。
...
![ブラウザでデバイス認証のためコード一致を確認される](image.png)

![LinqPadから実行したのでその許可を求める表示になる](image-1.png)

![LinqPadにアクセス許可を与えた状態](image-2.png)
...
AWS CLIでSSOログインを行うと、`.aws/sso/cache/xxxxx.json`に一時認証情報が書き込まれます。これを利用する方法です。
...
NuGetパッケージはC#アプリケーションでSSOログインするときと同じパッケージです。こちらも同様に、`AWSSDK.SSOOIDC`はなくてもコンパイルできるのですが、ないと実行時に無応答になるので必ず含めましょう。
