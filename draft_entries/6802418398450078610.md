...
コミュニティにあるとはいえ、広く使われているかというと微妙なところでしょう。理由はいくつもあるでしょうが、例えば`dotnet`標準コマンドでないためotnetツールを追加インストールが必要だったり、ツール毎のメタデータ表現も方言があって覚えるのが億劫宇です。また、インテリセンスやデバッグサポートが弱いなどの問題がありました。
