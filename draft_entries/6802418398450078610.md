...
これまでのC#でも.csファイルとプロジェクトファイル(.csproj)を組み合わせてあれば、`dotnet run Foo.csproj`のようにdotnet runコマンドで実行できました。
.NET 10から、プロジェクトファイルなしに単一のC#ファイルを`dotnet run app.cs`のように直接実行できるようになりました。
今回追加される後者はfile-based program(ファイルベースプログラム)と呼ばれ、従来の前者はproject-based program(プロジェクトベースプログラム)と呼ばれます。

# 従来のプロジェクトファイルなしのC#実行方法

これまでもC#には単一ファイルで実行する可能性が[CS-Script](https://github.com/oleg-shilo/cs-script)や[dotnet-script](https://github.com/dotnet-script/dotnet-script)など色々探られてきました。
...
しかし`dotnet`標準コマンドでないため、追加のdotnetツールインストールが必要だったり、方言があります。また、インテリセンスやデバッグサポートが弱いなどの問題がありました。
dotnetコマンド標準のプロジェクトベースの実行方法`dotnet run app.csproj`に慣れるに従って、dotnetコマンド標準で単一.csファイルを実行できないかと考えていました。

# dotnet run app.csを実行してみよう

早速簡単なC#ファイル`app.cs`を作成して、`dotnet run app.cs`で実行してみましょう。
...
これらは従来.csprojファイルで指定していましたが、ファイルベースプログラム用に.csファイル単独で実行できるようになりました。
...
これまでも、ConsoleAppFrameworkを使うことでC#プロジェクトとして管理しつつZxでシェルスクリプトっぽくかくことができました。ユニットテストで挙動担保しつつ、ちゃんと管理できるのがいい点ですね。ConsoleAppFrameworkははいいものですし、csprojで多くのバッチをまとめて管理もチーム管理として価値があります。

しかし、shebangをつかうことでシェルスクリプトを同じ感覚でスクリプトを実行しつつ、C#で書いて実行できるのは代えがたいメリットです。
シェルスクリプト代替としては、Zxを使って雑にC#スクリプトを書いてdotnet run app.csは直線でたどりつけます。
さらに、後述する`dotnet project convert app.cs`でプロジェクト化して、大規模になったスクリプトをConsoleAppFrameworkで管理する未来まで見据えられます。

Zx + ConsoleAppFrameworkの組み合わせは始めるのがどうしても億劫でしたが、これでシェルスクリプトではなく.csでもいい選択が現実になったのは嬉しいです。

## プロジェクト化する

.csxファイルで困ったのが、いざプロジェクト化したいときにcsprojファイルを用意してMSBuildプロパティを設定して、拡張子を.csに変えて、ビルドしてうまくいかなくて直して...を繰り返すことした。
ファイルベースプログラムは、これを一発で行う`dotnet project convert app.cs`コマンドを提供します。
...
![リストアされたNuGetファイルのパス](image-1.png)
...
ドキュメントを見るとサブディレクトリ名は「エントリーポイントファイルパスのハッシュ値を使っている」と記載があります。簡単に検証したところ、ファイル名までのフルパスを`ToUpper()`してSHA256を計算した値がディレクトリ名になっていることが確認できました。。
...
挙動をのぞいてきましたが、実際のソースコードを見てみましょう。
幸い、dotnet/sdkのソースコードはオープンソースで公開されており、GitHubで確認できます。

## エントリーポイント検出からビルドまでの流れ

具体的なコードで推測があっているか見てみましょう。dotnet/sdkのソースコードを見ると、主に2つのクラスで実装されています。
