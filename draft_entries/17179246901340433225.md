---
Title: "GitHub Actionsなどcronスケジュールの設定はLLMとやる話"
Categories:
  - GitHubActions
  - LLM
...
例えば、1日1回午前3時に実行する場合は以下のように指定します。
...
crontab.guruで調べると「At 03:00」と簡単に検証できます。

![crontabで検証](image.png)
...
![GitHub Actionsのスケジュール](image-2.png)
...
LLMにコメントを書きつつコンプリーションさせるといい感じに生成してくれます。GitHub Actionsのスケジュールで設定するなら、次のようにcron部分の上にコメント[^1]を書いておけばいいです。

![隔週のコンプリーション例](image-3.png)
...
これって、Copilotレビューも通じるということなので、GitHub Actionsのスケジュールには意図する時刻をコメント付与しておいてうまく使っていくと良いでしょう。


[^1]: 以前のCopilotは`# q:`のようなコメントがオススメでしたが、今はただのコメントで意図を汲んでくれます。
