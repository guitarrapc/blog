...
Terraformを使い始めてしばらくすると出会う課題が、プロジェクトでTerraform使用するバージョンを制約しつつ利用者へ透過的に展開する方法です。
Terraformはバージョンアップによって挙動が変わりえるため、プロジェクトで利用するバージョンはある程度固定したくなります。1つのTerraformですべての環境をそろえるのは難しいでしょう。自環境とプロジェクトで異なるバージョンのTerraformを使っていることもあるでしょうし、Terraformアップグレード時にそのブランチだけ異なるバージョンを使いたくなります。
