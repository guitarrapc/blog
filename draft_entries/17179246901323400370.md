...
多くのツールとクラウドを組み合わせ+連携してようやく本格的な運用ができる、この運用の難しさこそがKubernetesの難しさたる一面であることは間違いなくあります。普段運用していると気づきにくいですが、ふと振り返ったり、誰かに説明したり、オンボーディングをサポートするときに割と本当に難しいと感じます。そもそもコンテナの知識、それを運用するオーケストレーターの知識、クラウドと連携する方法、DNSやクラウドリースやセキュリティ、監視などなど、Kubernetesを運用するには多くの知識が必要です。この状況はKubernetesが登場して以来、複雑になる一方で、初心者がKubernetesを使い始めるのは非常にハードル高い状態が続いています。
...
これはDevtronの謳い文句ですが、DevtronはKubernetesのアプリケーションとインフラ管理を統合しAIアシスタントが運用を簡素化し、デリバリーを加速することを目的としているようです。
...
- CI/CD統合 = ビルドをDevtronが制御、そのビルド結果をDevtronが制御するArgoCD/Fluxでデプロイして、デプロイ履歴やデプロイ問題をAIが検知、自動ロールバック
...
Devtronは[Helmチャート](https://artifacthub.io/packages/helm/devtron-labs/devtron-operator)を使って導入します。まずはHelmリポジトリを追加します。
...
Devtronは内部で利用しているpostgresql podでストレージが必要なので、EKS AutomodeならEBS CSIドライバーがデフォルトで入っているのでサクッと行けます。
...
準備ができたら、Devtronをインストールします。少し初期設定から変更したい点をvalues.yamlで定義します。cicdモジュールは入れます、入れないでやっても入れない理由なかったので入れましょう。初期状態はDevtron Podに対してLoadBalancerを作成するので、ClusterIPを指定してport-forwardでアクセスします。もしいろいろなユーザーに展開して利用するなら、`ingress.enabled: true`でALBを作成すると良いでしょう。
...
これで導入すると徐々にpodが起動してきます。podの依存関係や起動タイミングの問題でスムーズにさっと起動しません。なんどかErrorやCrashLoopBackOffになることがありますが、10-15分ほど待つと最終的に安定します。
...
ちなみにOIDC認証する場合、権限を定義してグループやユーザーに紐づけることで操作対象の制約もできます。
...
Global Configurationは、Devtron全体の設定を調整します。例えば、クラスター登録、チャートリポジトリ登録、Gitリポジトリの登録や、イメージ/OCIレジストリの登録、ユーザーや認証回りの設定です。
...
例えば、argocdを追加すると次のようなpodが起動します。
...
DevtronはKubernetesの運用を大幅に簡素化しようとしていますが、触っていてなかなか難しいと感じる部分が多く感想です。

現在のDevtronは3つのワークロードが考えられます。
...
今回は、AWS EKSを使う前提で考えます。

**Devtron内部でCI/CDを組む**

GitOpsは、CIでビルドしてコンテナイメージをECRに配置、これを参照するKubernetes定義をシングルソースとしてGitHubなどに配置、CDとしてArgoCD/FluxでKubernetesにデプロイ、というシナリオが一般的です。Devtronはこれをさらに一歩進めて、CIもDevtronで行うことを想定しています。つまりDevtronでGit CloneしてDockerビルド -> レジストリに登録 -> これをさらにArgoCDなどで展開というシナリオです。GitOpsだけじゃなくて、CI/CDがセットになるのが特徴と言えます。Dockerfileでのビルド以外にDockerファイルなしのシナリオもDevtronにありますが、それはTyepScript/Python/PHPなどで.NETやGoはありません。まぁそんなものです、知ってた。

EKS的に難しいのが、DevtronからECRへのPushが必須で、この時の認証はEC2 Instance ProfileかAccessKey/AccesSecretしか想定になさそうなことです。AWS EKSは最低限IRSA、可能ならIAM Pod Identityで認証をとるのが一般的です。特にEKS AutomodeはIMDSv2ですがホップ調整ができないため、EC2 Instance Profileから認証をとってくるのは設計的に難しいです。EKS使うなら自前KarpenterなどのManaged Nodeじゃないときついわけです。認証系を各クラウド事情に沿ってアップデートしていくのは、Devtron自身の開発リソースには負担となりそうですね。がんばってほしい。

**ユーザー管理**

DevtronのUser管理は、原則OIDCからする想定です。ただ[前の記事](https://tech.guitarrapc.com/entry/2025/11/19/000000)で書いた通り、DexのOIDC認証をサブパスで隠すためグローバルな単一エンドポイントが必須です。この設計は、Devtronから自身が内包するDexへのアクセスにこのグローバル単一エンドポイントを使うため、エンドポイントのIP制限にDevtron Podが起動しているEC2のIP許可という制約が伴います。NAT GWは以下ならNAT GWでいい、そうじゃないなら次々と入れ替わるノードのIPを許可しないといけません。Devtronを触るなら、まずはこのあたりの設計を考える必要があります。

**ダッシュボード**

Kubernetesダッシュボードとしては貧弱と言わざるを得ません。
ここから情報を読み取ろうにも読み取れる情報少なすぎるので、今のダッシュボードだと運用的には微妙です。
AIインテグレート用のコマンドパネルはあるので、そういうところはよさそうだけど。
...
Kubernetes運用してていつも悩むんですが、運用系のコントローラーってPodが増えやすいので、もう少しPod数自体押さえたいんですよね。1コントローラー1Podだとすごいんですが、なんだかんだで冗長化で2Podが最低限ですし。
