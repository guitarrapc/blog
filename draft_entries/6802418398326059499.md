...
図にすると次のような構成です。ポイントは「ホストからRedis Clusterの各ノードにはホスト名で直接接続できない」制約です。Docker Compose内で解決される各ホスト名を使ってRedis Clusterの各ノードをつないでいるため、スロット先の接続先がこのホスト名になります。このホスト名はホストマシンからは解決できないというわけです。
