...
aquaは、aqua-proxy経由で実行するためか似たような状況が別のコマンドでも発生した記憶があります。詳細は忘れましたが、
今回は対処する必要があったので、なるほどtty...しょうがないのですが、シンボリックリンクで取りまわす方が好みなのでaqua-proxyを使わない別オプションがほしい気持ちがたびたび湧きます。
