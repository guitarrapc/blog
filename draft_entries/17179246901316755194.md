...
AWS Load Balancer ControllerでingressからALBが作成できないため、eventを確認したところ、次のようなエラーが出ていました。
...
AWS Load Balancer Controllerでサブネットがオートディスカバリーできない場合、多くの場合は次のどれかが該当します。

- サブネットに適切なタグが付与されていない (インターネット向けALBなら`kubernetes.io/role/elb : 1`のタグがサブネットに必要)
...
サブネットを見ると、1つのサブネットが残り6つのIPアドレスしかない状況でした。
実験用EKSなので汎用サブネットは`/24`[^1]で切っていましたが、`/22`程度に広げるとイイデスネ。

## まとめ

EKSのALBトラブルといえば、99％はサブネットのタグ付け忘れですがIPアドレス不足が原因でした。
今まで割と当たったことなかったのですが、こういうこともあるということで。
