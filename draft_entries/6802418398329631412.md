...
ただTerraform 1.10.0時点では、Data Sourceの置き換えとしてのEpehemeral Resourceであって、通常のリソースにパスワード的なものをStateを残さないようにはできません。

今回は1.11.0で追加される[Write-only Arguments](https://developer.hashicorp.com/terraform/plugin/sdkv2/resources/write-only-arguments)を使って、通常のリソースでもStateに値を残さなくするメモです。
...
* Terraform 1.11から各種リソースがWrite-only attributeを用意してくれれば、Write-only argumentによってtfstateから値を消すことができるようになる
* Write-only attributeにはただの文字列を渡せる、Ephemeral variableも渡せる
* 従来のvalueからvalue_woに変更する時はchangedであることが期待できる。


# Write-only Argumentsの動作とStateを確認する
...
## Write-only Argumentを使って書いてみる

はじめにどのように書くのか見てみましょう。
AWS Provider 5.87.0でWrite-only attributes[^1]が`aws_ssm_parameter`と`aws_rds_cluster`の2リソースに追加されています。Write-only attributesはリソースごとに対応が必要なので、AWS Providerの対応待ちになるリソースがしばらく多そうです。

コードを見ると大きな特徴が2つあります。このあたりは[ドキュメント](https://developer.hashicorp.com/terraform/plugin/sdkv2/resources/write-only-arguments)通りです。

1. 書き込み専用のattribute`value_wo`が従来のattribute`value`とは別に用意されている
2. ペアになるversion attributeが用意されている[^2]
...
System Managerパラメーターストアを見ると指定通り設定されています。Stateに
...
いくつか気になるパターンがありますね。一度先ほどのssmは消してから試します。
...
`value_wo`に渡すvariablesは`ephemeral`をfalseにした「ただの文字列」で実行してみましょう。
...
Stateファイルのスクショですが、`value`に値が入っちゃっていますね。
...
変更検知されて、SSMパラメーターも期待通り更新していますね。
...
[^1]: AWS Providerではwrite-only attributeと呼ばれていますが、同じことをさしているようです。呼び方的にはattributesの方が自然に感じます
[^2]: 書き込み専用attributeはStateで違いを検知できないので、このバージョンを用いて書き込み専用引数の使用をトリガーします。
