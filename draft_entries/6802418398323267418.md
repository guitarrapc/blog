...
Draft: true
CustomPath: 2025/01/25/235900
---

C#のRedisクライアントライブラリといえば[StackExchange.Redis](https://github.com/StackExchange/StackExchange.Redis)が定番です。StackExchange.Redisを使っていて困るあるある筆頭は、Redisのフェイルオーバーではないでしょうか。今回は、ElastiCache for RedisやMemoryDB for Redisでフェイルオーバーが発生した時に、C#アプリケーションを追随させる方法を紹介します。[^1]
...
クラスターモードでは、クラスターの中にN個のシャードがあり、シャードは1つのプライマリとM個のレプリカが存在します。非クラスターモードでは、シャードを持つクラスターが複数作成され、それぞれのクラスターにはプライマリとレプリカが存在します。[^2]
...
クラスターモードが管理するクラスターは1つです。クラスター内のシャードごとにデータがパーティショニングされバックアップは単一スナップショットになります。非クラスターモードではクラスターごとにバックアップを持ちます。
スナップショット1つで済むか、クラスターごとにスナップショットを選んで復元するか、どちらが楽かはいうまでもないでしょう。
