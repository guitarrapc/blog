...
C#のRedisクライアントライブラリ[StackExchange.Redis](https://github.com/StackExchange/StackExchange.Redis)を使っていて困るのが、Redisのフェイルオーバーです。
今回は、ElastiCache for RedisやMemoryDBでRedis Clusterモードを利用しているときに、クラスターのフェイルオーバーにアプリケーション側で追随する方法を紹介します。[^1]
...
クラスターモードとは何か、ってなりそうなのでザクっと説明します。

ElastiCache for Redisにはクラスターモードと非クラスターモードがあります。MemoryDB for Redisはクラスターモードのみの提供となっています。クラスターモードを有効にするかどうかは、スケール耐性、アプリケーションからの透過性、管理の容易さで決めるでしょう。言い換えると運用負荷ですね。私の経験上、本番環境でクラスターモードを用いない理由はほぼない認識です。

**スケール耐性**
クラスターモードでは、シャードを持つクラスターが複数作成され、それぞれのクラスターにはプライマリとレプリカが存在します。非クラスターモードでは、単一のエンドポイントに対してプライマリとレプリカが存在します。[^2]クラスターモードではシャードを追加することでデータがスロット分割されて書き込み・読み込みスケールできます。一方、非クラスターモードは1クラスターにレプリカは追加できますがプライマリは1つなので、書き込みスケールするにはクラスターを追加するしかありません。

なお、サーバーレスはクラスターモードのみの提供となっています。[^3]

**アプリケーションからの透過性**
クラスターモードではシャードを増減してもエンドポイントは常に1つですが、非クラスターモードではクラスターの数だけエンドポイントが複数存在します。
エンドポイントが増えるということは、アプリケーション的にはコネクションが別になります。コネクションが増えるということはアプリケーションの再デプロイが必要になったり、データをどのように分散させるか考慮する必要が出てくるでしょう。

**管理の容易さ**
クラスター内のシャードごとにデータがパーティショニングされバックアップは単一スナップショットになります。非クラスターではクラスターごとにバックアップを持ちます。スナップショット1つで済むか、クラスターごとにスナップショットを選んで復元するか、どちらが楽かはいうまでもないでしょう。

| 基準 | クラスターモード | 非クラスターモード |
| --- | --- | --- |
| 書き込み・読み込み負荷をスケール | 〇 | 書込み×, 読み込み: 〇 |
| エンドポイントの数 | 単一エンドポイント | Redisクラスター分で可変 |
| スナップショットの数 | 1つ | Redisクラスター分で可変 |

[AWSドキュメント](https://docs.aws.amazon.com/AmazonElastiCache/latest/dg/Replication.Redis-RedisCluster.html)の図を引用すると、両社は次のような配置の違いです。

![image](https://github.com/user-attachments/assets/9565ffef-752d-40fc-bc5a-9a5066446e97)
> 引用: Replication: Valkey and Redis OSS Cluster Mode Disabled vs. Enabled - Amazon ElastiCache User Guide
...
Azure Managed RedisのMicrosoft Learnには、[接続の回復力に関するベスト プラクティス - Azure Cache for Redis | Microsoft Learn](https://learn.microsoft.com/ja-jp/azure/azure-cache-for-redis/cache-best-practices-connection#using-forcereconnect-with-stackexchangeredis)という記事があります。Azure Managed Redisを前提にしていますが、StackExchange.Redisを使った接続を前提にしているので、これを読んでおくといいでしょう。

また、この記事で紹介されている通り、StackExchange.Redisを直接使うのではなく、[Microsoft.Extensions.Caching.StackExchangeRedis](https://www.nuget.org/packages/microsoft.extensions.caching.stackexchangeredis)を使うと`UseForceReconnect`プロパティをtrueに設定するとコネクション再接続はしてくれます。ただし、このライブラリでフェイルオーバーさせたことはないので、本記事では触れません。
...
3. 自動復旧中にASP.NET Coreのサーバー終了なら自動復旧は止めたいので`IHostApplicationLifetime`を利用して`ApplicationStopping`のキャンセルトークンを利用している
4. OnConnectionFailedは逐次処理が望ましいはずなので意図的に同期コードにしている

利用時は次のようになります。ASP.NET CoreなどDIを想定しているので、コンソールアプリケーションで使うときはいい感じに変更してください。DI部分を外しても適当に使えます。[^5]
