...
- GitHub Copilotにどう翻訳をさせるかは、インストラクションで指示しIssue本文で調整できる。

## 多言語翻訳は維持が難しい

ドキュメント管理で難しいことの一つが多言語対応です。
...
このような多言語翻訳を維持する一つの手段は、LLM（大規模言語モデル）による翻訳補助です。
...
[f:id:guitarrapc_tech:20251224181808p:plain:alt=GitHub Copilotのユースケース例]

ただ、何も補助なしでCopilotエージェントに翻訳を任せるのは微妙です。

- どこまで翻訳するか、どういう基準で翻訳するかが安定しない
- コードやリンクなど意図しない箇所が翻訳されることがある
- 固有語句の翻訳ニュアンスがずれることがある
...
翻訳作業を記録・任意の指示を行うため、GitHub Issueを使ってCopilotエージェントに翻訳タスクを割り当てると、翻訳PRで結果をレビューしやすくなります。
...
- CopilotエージェントがIssue内容に基づき、ドキュメント翻訳を実行する
...
インストラクションは、「翻訳対象ドキュメントとその言語判断を指示する」「ドキュメント後の翻訳スタイルや一貫性を具体的に指示する」を重視しています。どういうタスクにするかは、Issueで調整するので、インストラクションは翻訳品質や基準に集中させます。
...
`document translation`ワークフローを`workflow_dispatch`で手動実行します。

[f:id:guitarrapc_tech:20251224181816p:plain:alt=workflow_dispatchで実行]

[f:id:guitarrapc_tech:20251224181823p:plain:alt=issueジョブが実行]

[Issue](https://github.com/guitarrapc/scoop-bucket/issues/19)が作成され、`documentation`ラベルが付与されます。

[f:id:guitarrapc_tech:20251224181831p:plain:alt=Issue作成、documentationラベルを付与]

`documentation`ラベルが付与されると、再び`document translation`ワークフローがトリガーされ、今度は`assign`ジョブでIssueにCopilotエージェントがアサインされます。

[f:id:guitarrapc_tech:20251224181840p:plain:alt=issueにCopilotがアサインされる]

CopilotエージェントがIssue内容に基づき、ドキュメント翻訳を実行しPRを作成します。今回はIssueボディが次の内容なので、README.md（英語）とREADME-ja.md（日本語）の翻訳が行われます。
...
[f:id:guitarrapc_tech:20251224181848p:plain:alt=CopilotがPRを作成]

Copilotエージェントが作成した[PR](https://github.com/guitarrapc/scoop-bucket/pull/20)は次の通りです。

[f:id:guitarrapc_tech:20251224181855p:plain:alt=翻訳対応PR]

ファイル差分を見ると、いい感じに翻訳されています。あとは、翻訳内容をレビューして、必要に応じて修正し、マージすれば完了です。

[f:id:guitarrapc_tech:20251224181905p:plain:alt=PRのファイル差分]

## インストラクションでLanguage Consistencyを指示する効果

先に示したインストラクションで、特にLanguage Consistencyの部分が便利です。この指示を与えることで、ドキュメント全体の言語を統一し、技術要素のみ英語のままにすることができます。

LLMによる翻訳は、文書が意外と漏れます。例えば、日本語文書の中に、`These elements should .....`のような英語文書が混じっていても、翻訳されずに残ってしまうことがよくあります。また、ある言語にPRで加えられた変更を他言語に反映させる場合、PRになかった箇所に異なる言語が混じっていても翻訳されず残るのですが、このインストラクションで防げます。

このインストラクションがない場合、次のようにPRや直近コミットで変更を加えていない箇所が翻訳されずに残ってしまいます。

[f:id:guitarrapc_tech:20251224181916p:plain:alt=インストラクションを入れない場合]

同インストラクションを入れると、PRや直近コミットになくても、ドキュメント全体の言語を統一してくれます。

[f:id:guitarrapc_tech:20251224181924p:plain:alt=インストラクションを入れた場合]
