---
Title: DevtronにみるDexをアプリケーションの裏に組み込みしてOIDC連携する際の制約
Category:
- Kubernetes
...
ただ近年はDex離れも見られ、当初Dexを使ってOIDC認証を提供していたものの、独自OIDC認証実装へ移行しているものもあります。例えば[ArgoCD](https://github.com/argoproj/argo-cd)は現在、Bundled Dexを使ったSSO認証と、外部OIDCプロバイダーへの直接連携の両方をサポートしています。ただしArgoCDの場合、Dexを使用する際も独立ホスト方式(`/api/dex`パス)で提供しており、DevtronとはDexの統合方法が異なります。個人的にも、Dexを使っているアプリケーションのOIDC実装次第でOIDC連携を思ったようにできないケースがあり、避けたい気持ちもあります。ということで、Dexを使っているアプリケーションのOIDC連携の制約として、Devtronを例に紹介します。
...
Devtronは、OIDC認証を設定することで任意のIdPでSSOログインできます。DevtronのOIDC認証設定はDexであり、OIDCあるなるな設定要素が並んでいます。以下は、CognitoをIdPとして利用しつつ、DevtronをIngressで公開している場合の設定例です。
...
Devtronに組み込まれているDexもこのエンドポイントを提供しており、例えば`https://devtron.example.com/orchestrator/api/dex/.well-known/openid-configuration`エンドポイントにアクセスすると、次のようなOIDCプロバイダーJSONレスポンスが返ってきます。
...
エラーは、DevtronのpodからdevのOIDCエンドポイント`http://localhost:8082/orchestrator/api/dex/.well-known/openid-configuration`へアクセスするときに発生しています。なぜなら、`localhost:8082`はブラウザから見たDevtronのエンドポイントですが、Devtronのpodから見た`localhost:8082`はDevtronのpod自身を指しており、Devtronのpod内で動作しているDexエンドポイントにはアクセスできないからです。
...
エラーは、リダイレクトURIの検証時に発生しています。クラスター内部のSVC URLを用いることでDevtronのPodからDevエンドポイントに疎通できるのですが、リダイレクトURI`http://localhost:8082`とDex URL`http://devtron-service.devtroncd.svc.cluster.local/orchestrator`のhost+portが違うので検証エラーになっています。
