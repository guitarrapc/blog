...
`actions/download-artifact`で、`run-id`で対象のワークフロー履歴を指定 + `github-token`で認証情報を渡せば、他のワークフローやリポジトリのアーティファクトをダウンロードできます。

例えば、次のように書くと、そのワークフローでアップロードしたすべてのアーティファクトをダウンロードします。
...
ドキュメントには`actions: read`権限をジョブにつけるように書かれていますが、同一リポジトリの場合は権限を指定しなくてもダウンロードできます。
...
![ワークフロー実行結果にoutput.tar.gzが添付されている](image.png)<!--image.png-->
...
![workflow_dispatchで実行する](image-2.png)<!--image-2.png-->

実行履歴をみるとうまく、指定したワークフローの最新run-id`21203098493`からダウンロードできています。

![actions/download-artifactで他ワークフローのアーティファクトをダウンロードできている](image-1.png)<!--image-1.png-->
...
![期限が切れている](image-3.png)

## まとめ

以前の`actions/download-artifact`は同一ワークフローからしか取得できなかったので、てっきり今もかと考えていましたが、Blameを見ると3年ほど前から他のワークフローやリポジトリのアーティファクトもダウンロードできるようになっていました。

中にはGitHub APIを駆使して取得する例もありますが、今は`actions/download-artifact`で簡単にできるのでこっちを使うといいでしょう。
