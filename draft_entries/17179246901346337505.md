...
例えば私の場合、あるワークフローでネイティブビルドを行っていて10分かかります。そのビルド成果物を別のワークフローで使い回したいときに、一度ビルドしたアーティファクトを他のワークフローでダウンロードして使いたいというケースでした。ビルドを何度も繰り返すのは無駄なので。

今回は、actions/download-artifactでそれができるという話です。シラナカッタ。

[:contents]

## 概要

`actions/download-artifact`([v4以降](https://github.com/actions/download-artifact/releases/tag/v4.0.0)、[v3とv4の差分](https://github.com/actions/download-artifact/compare/v3...v4.0.0))で、`run-id`で対象のワークフロー履歴を指定 + `github-token`で認証情報を渡せば、他のワークフローやリポジトリのアーティファクトをダウンロードできます。例えば、次のように書くと、そのワークフローでアップロードしたすべてのアーティファクトをダウンロードします。
...
- `run-id`: 対象ワークフローの履歴ページURLの末尾にある数字列。例えば、`https://github.com/guitarrapc/githubactions-lab/actions/runs/21203122941`なら`21203122941`
...
私の`guitarrapc/githubactions-lab`リポジトリを使って実際の挙動を見つつ使い方を確認しましょう。
...
次に、上記ワークフローでアップロードしたアーティファクトをダウンロードするワークフロー`artifact-other-workflow.yaml`を用意します。

`artifacts-other-workflow.yaml`は、指定したワークフローからアーティファクトをダウンロードする例です。`run-id`を指定した場合はその履歴から、指定しなかった場合は最新の履歴からアーティファクトをダウンロードします。
...
これを実行します。`run-id`を指定してもいいですが、今回は自動判別させます。

[f:id:guitarrapc_tech:20260121183104p:plain:alt=workflow_dispatchで実行する]<!--image-2.png-->

実行履歴をみるとうまく、指定したワークフローの最新run-id`21203098493`からダウンロードできています。
...
私はよく`retention-days: 1`にしているため、翌日にはアーティファクトが消えていて見つからなくなります。このため、アーティファクトが見つからない場合は、対象のrun-idのアーティファクトがまだ存在しているか確認してください。

期限が切れている場合、次のように`Expired`と表示されます。

[f:id:guitarrapc_tech:20260121183126p:plain:alt=期限が切れている]

## まとめ

以前の`actions/download-artifact`は同一ワークフローからしか取得できなかったので、てっきり今もかと考えていましたが、v4なので3年ほど前から他のワークフローやリポジトリのアーティファクトもダウンロードできます。

中にはGitHub APIを駆使して取得する例もありますが、今は`actions/download-artifact`で簡単にできます。
