...
軽減策を考えてみましたが多いです。このうちA, C, D, F, Gは静的解析ツールで自動検知できるので優先度高めに対応します。Bはユーザー/オーガニゼーションで一度設定すればOKそうです。E, H, I,Jはリポジトリごとに設定が必要です。
...
静的解析ツールで設定を確認/調整、自動化を検討。次いでユーザー/オーガニゼーションの設定見直し、リポジトリごとの設定、という順番で対応してみましょう。

# 静的解析ツールと自動化

真っ先に取り組むべきは静的解析ツールによる自動化です。リポジトリを複数管理していると手動でワークフローを調整するのは無理です。時間は有限なので、できるだけ自動化して見落としを防ぎつつ精度を高めましょう。私のおすすめは、次の流れです。
...
actionlint自体の良さと、bash自体に疲弊するバランスが難しい。

## ghalint

ghalintはGitHub Actionsのセキュリティチェックを行うツールです。ghalintはGitHub Actionsのワークフローの簡易的なセキュリティチェックを行います。Goで書かれているので後述するzizmorより動かしやすいのと、一番気になるところはこれでカバーできるので、まずはghalintを使うのがいいでしょう。野良のワークフローはほぼできていないので、どこを見るといいか目を養えます。
...
記法に依存した指摘があるのは微妙です。actionlintが嬉しい理由に戻るのですが、複数の書き方がある時に、この書き方がよりよいというガイドを示すのは割といい派です。

例えば、ジョブが1つならワークフローレベルの`permissions`を指定しても問題ないのですが、ジョブが複数ある場合はワークフローレベルの`permissions`を指定してもエラーになります。これは書き方に依存する[^5]ので、ジョブに一本化したほうがいいんじゃないのかなぁと感じます。

## zizmor

zizmorはGitHub Actionsのセキュリティチェックを行うツールで、ghalintよりも[チェックが厳し目](https://woodruffw.github.io/zizmor/audits/)です。Rustで書かれているのですが、各プラットフォーム向けのバイナリインストールは手薄です。ただDocker経由で実行できるので、私は次のように実行しています。
...
ghalintではエラーにならないものも、zizmorで気づけるのは嬉しいです。例えばpull_request_targetイベントは、GitHub Actionsの`GITHUB_TOKEN`を使ってpushできるので、悪意のあるコードが仕込まれた場合にpushされてしまう可能性があります。zizmorはこのような危険なトリガーを検出してくれます。
...
修正してみて、エラーになるのがつらかったポイントを紹介します。
...
次のようなaqua.yamlをリポジトリルートに設定しておきます。
...
GitHub Actionsワークフローを次のように設定しておくと、GitHub Actionsで静的解析ツールを自動実行できます。
