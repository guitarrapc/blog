...
そもそもCloudFormation Stackで何も問題ないならポートする必要はありません。しかし、現状のCloudFormation Stackはそのまま使うのに躊躇う理由が2つあります。
...
毎回diffが出るのは地味にいやです。IgnoreChangesで無視するにもDatadogApiKeyで発生するので避けたいところです。CloudFormationが他のIaCより面倒なのがテンプレートはUrl参照が主流なことです。[^1]このため、テンプレートを修正したくても手間で気が進みません。コードでかかれていたりテンプレートYAMLが直接書かれているなら一瞬なのに!!

幸いCloudFormationに大きな変更が入ることは考えにくいため、こういうわずらわしさがあるならPulumiにポートしてしまおうというのが今回の動機です。

# Pulumi C#でどう書き進めるか

2つの方法があります。1つは生成AIにCloudFormationからPulumiコードに変換させることです。[^2]もう1つはCloudFormation Stackでできたリソースやコードを参考に手で書くことです。CloudWatch StreamからHttp Endpointにどうやって送るか気になるので、今回は後者を選択します。
...
public static class DatadogMetricStream
{
    public static void Create(string name, DatadogMetricStreamArgs args)
    {
        ...
        var firehoseDeliveryName = $"{name}-delivery";
          ...
          "Statement": [
            {
                ...
                "Resource": [
                  "arn:aws:firehose:*:{{x}}:deliverystream/{{firehoseDeliveryName}}"
                ]
            }
          ]
        }
        ...
        // this region
        {
            ...
            CreateResource(name, accountId, region, "", firehoseIam.Arn, metricStreamIam.Arn, bucketNamePrefix, firehoseDeliveryName, opt, args);
        }
        ...
        foreach (var (region, provider) in args.AdditionalProviders)
        {
            ...
            CreateResource(name, accountId, Output.Create(region), $"-{region}", firehoseIam.Arn, metricStreamIam.Arn, bucketNamePrefix, firehoseDeliveryName, opt, args);
        }
    }
...
動作させてみましょう。ap-northeast-1リージョンとus-east-1リージョンでCloudWatch Metric Streamを作成、Datadogに送信するコードです。
