...
ということは、Terraformのoutputはこういう感じで出力したいですね。ロール毎にannotationsごとのoutputを出力するのではなく、annotations配下にarnを並べた文字列として出したい。[^1]
...
今回の課題は、次のようなterraform配列から↑のような文字列を出力をoutput.tfで行うことです。
...
さて、String DirectivesとHeredocを使っていい感じの文字列を出力する方法がわかりました。それでは、実際にoutput.tfを書いてみましょう。
