...
ARM64対応もされており、AWSのGravitonやAzureのArm VM、Google CloudのArm VMを利用できるため、x86_64よりもさらに安価なLinuxサーバーを利用できます。x86_64とARM64はビルド時に切り替えるだけで、ほとんどのコードがそのまま動作するため、積極的に狙っていく価値があります。ARM64は経験上特に暗号系処理で10％10％程度パフォーマンスが落ちる傾向ですが、x86_64系よりコストが30-40％下がるため、トータルで有利です。
...
サーバーを動かすにあたりLinuxにはツールやミドルウェアが充実しています。C#サーバーをLinuxで動かすことで、これらを活用できるため、開発や運用が容易になります。これは明らかにWindowsで動作させるよりも有利で、いちいちWindowsで動かせるか気にすることなくノウハウがそのまま活用できます。
...
この辺りは、AIとの協調作業という観点でも重要です。何しろツールの情報が豊富かどうかは回答の精度に大きく影響します。Linuxで動かすことで、ツールの情報が豊富に得られ、AIを活用した開発や運用がしやすくなります。
...
C#は.NET Core(現在は.NET)においてLinuxで動作することを前提に進化してきたため、パフォーマンスや安定性も備わっています。実際、私の経験でもLinuxの方がパフォーマンスよく小さいリソースで安定動作します。
