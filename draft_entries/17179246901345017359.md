...
同一リポジトリでもReusable Workflowを使うことを否定はしていないので、そこが落としどころになりそうです。

### 改ざん不能の要件が足りなさそう

SLSAの改ざん不能について考えると、同一リリースやタグを上書き可能な場合、改ざん不能の要件が足りなさそうに思えます。例えば、GitHub Releasesで同一タグを上書きできる設定になっている場合、ビルド工程を丸っと乗っ取られた場合に、同じタグやリリースを後から上書きされる可能性があります。これを防ぐには、イミュータブルリリースが必要になります。
...
SLSAアテステーションレポートには、対象アーティファクトのハッシュ値が含まれ、利用者はそのビルドステップで生成されたものであることを検証できます。GitHub Actionsでやるなら1つステップを追加するだけで済みます。
...
SBOM/SLSAに関わらず、リリースが後から上書きされるのは利用者からすると疑いしかないので、GitHubのイミュータブルリリースは有効にしておくのがよさそうです。
