...
0.4.0まではリトライが動作していますが、0.4.1からリトライが機能しなくなっているようです。

Windows以外では、ロガーにtrx以外を用いることができません。

dotnet-retestが出力を制御するのですが、`--no-summary`指定時に表示が壊れたりします。また、テストプロジェクトに1000以上のテストを含まれていると[エラーが生じる](https://github.com/devlooped/dotnet-retest/issues/47)と報告されています。この場合、`--no-summary`を指定するとエラーを回避できるようですが、微妙なラインです。
