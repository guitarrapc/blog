...
C# (.NET)のプロジェクトで外部パッケージを管理する方法はいくつかありますが、Visual Studio 2022で導入されたCentral Package Managementは、プロジェクトのパッケージ管理を簡単にするための新しい方法です。特に複数のcsprojファイルを持つソリューションで、パッケージを一元管理し、パッケージのバージョンを一貫して管理できます。

しかし、複数.csprojでNuGetパッケージ定義をCentral Package Managementに移行する際には、手作業での移行が必要でした。Visual Studio 2022 17.1からは`.NET Upgrade Assistant`を使うとCentral Package Managementに移行できるので紹介します。
...
このプロジェクトをCentral Package Managementに移行すると次のような構成になります。
...
Central Package Managementでは、`Directory.Packages.props`ファイルにパッケージのバージョンを一元管理します。このファイルは、プロジェクトのルートディレクトリに配置されます。[^2]
...
csprojファイルは次のようになります。csprojで定義していたパッケージのバージョン情報がなくなっています。Central Package Management移行後はcsprojでパッケージのバージョンを`Verison`で指定するとエラーになりますが、`<PackageReference Include="PackageA" VersionOverride="3.0.0" />`のように`VersionOverride`を使うとcsprojでバージョン指定できます。管理が難しくなるので個別指定はおすすめしません。
...
`Directory.Packages.props`ファイルの`ManagePackageVersionsCentrally`と`PackageVersion`に注目してください。[^3] `ManagePackageVersionsCentrally`はCentral Package Managementを有効にするためのプロパティです。.csprojに直接定義するとそのcsprojだけをCentral Package Managementに移行できますが、アップグレードアシスタントを使うと、`Directory.Packages.props`に記載されて複数のcsprojを一括でCentral Package Managementに移行できます。`PackageVersion`はManagePackageVersionsCentrallyで指定したパッケージのバージョンを一元管理するためのプロパティです。

すべてのcsprojで使っているNuGetパッケージとバージョンが`Directory.Packages.props`に記載されているので、パッケージのバージョンを確認すればよくなりました。
...
[^3]: csprojでNuGetパッケージ参照するときは`PackageReference Include`でした
