...
TerraformのData Sourceは、Terraformの実行時にリソースを取得するためのものです。Data Sourceは取得した値をStateやplanファイルに保存しその値を使ってリソースを作成します。このため、Data Sourceを使う場合、planファイルやStateファイルに機密情報が残るため扱いに注意が必要です。
