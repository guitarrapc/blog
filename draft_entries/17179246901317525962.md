...
最初にアーキテクチャごとに、各OSでどうやったらCPUモデルを取得できるかをまとめます。
...
前回はx86_64環境のみの対応でしたが、今回はARM64環境も含めてC#でCPUモデルを取得する方法を紹介しました。今回はマッピングを書きましたが、ARM64環境でCPUモデルをとるのにマッピングテーブルを見る必要があるのは面倒なので`lscpu`コマンドを実行してもよいかもしれません。
