...
NuGetには[ロックファイル](https://devblogs.microsoft.com/dotnet/enable-repeatable-package-restores-using-a-lock-file/)(packages.lock.json)を用いてリストアする機能があります。ただ、C#のプロジェクトでロックファイルを使っている例はあまり見かけません。実際、GitHubでRestorePackagesWithLockFileをキーに[検索すると](https://github.com/search?q=RestorePackagesWithLockFile+language%3AXML&type=code&l=XML&p=1)7300件程度と、C#リポジトリ全体が6.9M件あることからすると少ないです。

最近SBOMについて調べる中で、なんでNuGetのロックファイルがあまり使われていないのか考えていたのでメモです。
...
[f:id:guitarrapc_tech:20260114182231p:plain:alt=NuGetのパッケージ依存関係]<!--image-1.png-->
...
ロックファイルは、[Central Package Management(以降CPM)](https://learn.microsoft.com/ja-jp/nuget/consume-packages/central-package-management)(Directory.Packages.props)が有効でも特別な対応はしません。つまり、`Directory.Packages.props`でバージョンを一元管理していても、ロックファイルは個々の.csprojパスに生成されます。
...
![Visual StudioのManage NuGet Packageでもバージョンを指定する](image-2.png)<!--image-2.png-->
...
![直接.csprojの編集が必要](image-3.png)<!--image-3.png-->
...
事前にどのようなパッケージが入るかの予定を示すロックファイルは、C#においてはあまり使われておらず、デフォルトもロックファイルを使う設定じゃないです。パッケージ指定文化やSBOMの視点で考えてみましたが、今のことろC#でロックファイルを使うメリットは小さいと考えています。

今後ソフトウェアサプライチェインのセキュリティが重要になる中で、ロックファイルの役割も見直される可能性はありますが、決定論的な保証をできないケースが思いつかなかったので見通せないですね。
