...
価格設定があります。4ノードまでは不良、それ以上は有料になるので注意です。小規模なクラスターでいろいろいじったりするのがいいです。
...
```sh
# デプロイ
$ kubectl apply -f k8s/test/guestbook.yaml
...
# 削除
$ kubectl delete -f k8s/test/guestbook.yaml
```
...
```sh
# デプロイ
$ kubectl apply -f k8s/test/bookinfo.yaml
...
# 削除
$ kubectl delete -f k8s/test/bookinfo.yaml
```
...
```sh
# デプロイ
$ kubectl apply -f k8s/test/etcd.yaml
...
# 削除
$ kubectl delete -f k8s/test/etcd.yaml
```
...
![alt text](image-23.png)<!-- image-23.png -->
...
![alt text](image-17.png)<!-- image-17.png -->
...
| Request | Response |
| --- | --- |
| ![alt text](image-18.png)<!-- image-18.png --> | ![alt text](image-19.png)<!-- image-19.png --> |

また、この時のサービスマップは、パスも表示されていてすごいです。

![alt text](image-20.png)<!-- image-20.png -->
...
DNSトラフィックを確認するには、`dns`をクエリに追加します。サーバー間通信でもDNSクエリは流れるので、うまく解決できないっていうときに絶大な効果を発揮します。
...
| Request | Response |
| --- | --- |
| ![alt text](image-14.png)<!-- image-14.png --> | ![alt text](image-13.png)<!-- image-13.png --> |
サービスマップで、このDNSトラフィックを確認できます。

![alt text](image-22.png)<!-- image-22.png -->
...
| Request | Response |
| --- | --- |
| ![alt text](image-15.png)<!-- image-15.png --> | ![alt text](image-16.png)<!-- image-16.png --> |
...
![alt text](image-21.png)<!-- image-21.png -->
