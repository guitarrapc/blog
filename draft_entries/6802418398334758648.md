...
```sh
$ dotnet publish -r linux-musl-x64 -c Release /t:PublishContainer -p:ContainerImageTag=1.0 -p:InvaliantGlobalization=false
$ docker image ls
REPOSITORY          TAG       IMAGE ID       CREATED             SIZE
webapiapp22         1.0       d235eede5af4   7 seconds ago       166MB
```

![image](https://github.com/user-attachments/assets/ba5add06-3353-4be5-9c4b-f9dd5d2606ad)
...
`dotnet publish`でイメージ作成する場合、[デフォルトラベル](https://learn.microsoft.com/en-us/dotnet/core/containers/publish-configuration#default-container-labels)が設定されます。
...
`USER`相当です。ItemGroupで`ContainerUser`を指定します。.NET 8から.NETのコンテナはnon-rootを果たしました。このため、rootではなく`app`ユーザー(UID/GIDは`1654`[^5])で動作します。
...
chiseledイメージにはICU/tzdataが含まれないため、tzdataへのアクセスができずエラーが発生しています。これはICUでも同様にトラブルを起こします。
