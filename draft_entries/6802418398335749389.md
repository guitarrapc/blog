...
この記事でEKS AutoModeなるほど、とならず実際に触って感触を確かめてみましょう。特に気になるEKS AutoModeの違いは次のポイントです。
...
一方で、AutoModeでも共通していることもあります。この辺りは安心ですね。
...
EC2にNameタグがつきません。Karpenterは`Name`タグにインスタンスの`IP name` (この例なら`ip-10-100-21-142.ap-northeast-1.compute.internal`)を設定します。(1段目の図)しかしEKS AutoModeは`Name`タグ自体が設定されません。(2段目の図)
...
EC2のボリュームも違います。KarpenterではEC2NodeClassでボリュームサイズを指定すると`/dev/xvda`のサイズが変わりました。しかしEKS AutoModeでは、EC2へストレージ`/dev/xvda`と`/dev/xvdb`の2つアタッチされ、NodeClassでボリュームサイズを指定すると`/dev/xvdb`のサイズが変わります。

![image](https://github.com/user-attachments/assets/8f5f6b8e-9bc9-46b0-aedf-9c58a916fb3e)

カスタムNodeClassで20GiBを指定すると、`/dev/xvdb`EBSのサイズが20GiBになっていますね。
...
[ドキュメント](https://docs.aws.amazon.com/eks/latest/userguide/create-node-class.html)がIAM Role ARNになっているので64文字に収まらずびっくりしますが、実際は名前指定なので大丈夫なはずです。
...
原因は、NodeClassで追加タグを設定していることです。この場合、[追加権限](https://docs.aws.amazon.com/eks/latest/userguide/auto-learn-iam.html#tag-prop)がEKS AutoModeクラスターのIAM Roleに必要です。IAM Roleを適切に設定すれば無事にNodeClaimが展開されて、EC2が起動、Podがスケジュールされます。
