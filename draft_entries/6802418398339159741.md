...
CustomPath: 2025/03/19/235900
---

Ansibleで`ansible.builtin.apt_repository`を使ってppaをインストールしていたのですが、Ubuntu 24.04からはエラーが出るようになりました。
ということで対処メモ。
...
`ansible.builtin.apt_key`を使ってキーを管理して、リポジトリ定義を deb 形式に変更すれば解決です。なるほど、apt-keyが非推奨確定。
...
原型が残らなくなったけど、こういうのはよくあるしaptは避けられるなら避けたいことが多いです。
