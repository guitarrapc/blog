...
C#のstraceを追いかけてみましたが、面白かったです。ただ、こうやってみていくとstraceのシステムコールが少ないからよい、とは一概には言えないと感じます。
仮にdotnetランタイムが`/proc`や`/sys`を見に行くシステムコールをしていない場合、それはGC、JITコンパイラのTiered PGO最適化、その他ランタイム最適化のための情報を取得できないことを意味します。CやRustのように、メモリ管理や最適化を手動で行う言語と異なり、C#などのマネージドランタイムは、ランタイムがシステムの状態を把握して最適化を行うことに依存しています。したがって、必要なシステムコールはある程度存在するでしょう。

C# AOTのシステムコールを見る限り、ランタイムにとって最低限必要なシステムコールに絞られているように見えるのは、Native AOTビルドとして頑張っているように感じます。一方で、JIT側はJITランタイムを含むため、より多くのシステムコールを行っています。このあたりは、AOTとJITの違いがよく出ていますね。
