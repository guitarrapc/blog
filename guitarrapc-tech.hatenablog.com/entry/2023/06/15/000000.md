...
モノレポを CI でビルドするにあたり、最初にして最大の課題がチェックアウトです。
先日GitHub Actions の `actions/checkout` でスパースチェックアウトできるようになったので、これを使ってモノレポのチェックアウトを高速化しました。
...
* スパースチェックアウトなら、モノレポのビルドでも特定のパスだけをチェックアウトできるのでビルド時間を短縮できる
* git は、シャロークローン(shallow-clone) と スパースチェックアウト (sparse checkout) を併用することで最速でチェックアウトができる
...
GitHub Actions のチェックアウトは、 [actions/checkout](https://github.com/actions/checkout) を使うのが定番です。特に理由がないなら、自分でチェックアウトするのではなく `actions/checkout` を使うことで、シャロークローンがデフォルトに有効になったり、サブモジュールやLFS のサポートもされます。
...
簡単にいうと、スパースチェックアウトはチェックアウトするファイル(パス)を指定することで、特定のファイルだけをチェックアウトすることができる機能です。
Git 2.25.0 から `git sparse-checkout` コマンドで使えるようになりました。

なぜ 「特定のファイルだけをチェックアウトすることができる」ことが嬉しいのか、先のブログの図を使って簡単に説明します。
...
`client/android` にあるファイルだけチェックアウトできれば、他のコードのチェックアウトが不要になり高速に完了しそうです。
...
シャロークローンを使うだけだとできなかった、必要なファイルだけをチェックアウトすることができました。

![image](https://user-images.githubusercontent.com/3856350/245855336-3063151d-896b-4b27-8e94-0cab335287aa.png)
> https://github.blog/2020-01-17-bring-your-monorepo-down-to-size-with-sparse-checkout/ より引用

スパースチェックアウトはシャロークローンと併用もできるので、最新1件の履歴のうち、必要なファイルだけチェックアウトすることができて最速を簡単に狙えます。
...
一見スパースチェックアウトをしているだけですが、`actions/checkout` はデフォルトでシャロークローン (depth=1) なので、スパースチェックアウトと併用することで最速でチェックアウトできます。簡単、便利で最高の体験だと思います。
...
手元のモノレポの一つでは、シャローンクローンを使ってもチェックアウトに 1m20s かかっていたのが、スパースチェックアウトを併用することで 1s～15s に改善しました。
1s は `.github` など最低限必要なパスだけをスパースチェックアウトした場合で、15s はビルドに必要なパスだけをスパースチェックアウトした場合でした。

一日に何度もCIが回り、複数のジョブで何度も actions/checkout を使う必要があったのがすべて高速化したので、CIの回転時間が大幅に改善されました。

意識しにくいですが、Billable minutes も大幅に減っているのでコスト管理的にも嬉しいですね！
財布に優しく、高速にCIを回せる、スパースチェックアウトは最高です。
