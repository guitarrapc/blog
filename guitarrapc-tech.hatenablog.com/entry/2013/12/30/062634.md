...
何度も登場するストリーム処理パターンを定義しておく。他言語でやる手法が、今ようやくPowerShellでも取り上げられつつあります。((今回は用途を限定しますが、汎用的な用途を今後示していければと思います。))

# まとめ

近くいくつかのパターンをまとめたいと思いますが、map や reduce などいわゆるあれな処理をまとめておいて、Moduleに組み込む。こういったやり方でコレクションを処理しやすくなります。

あ、そうそう。Filter は、PowerShell におけるパイプラインを通した繰り返し処理で最速の手段です。そう Foreach-Object や V3の省略記法 などよりずっと。Where-Objectなどのコレクションフィルタリングでも示しましたが思い出しておいてくださいね。
