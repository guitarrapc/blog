...
アンインストールされてコマンドが実行できなくなっていますね。ただ、コマンド実行時にaqua-proxyがaquaのパスにバイナリがないと言っているのは不思議な挙動です。アンインストールしたならaqua定義から消して、またaqua管理外になってほしいところです。[^3]
...
```sh
# アンインストール+定義削除後にこれなら自然
$ kubectl version --client
Command 'kubectl' not found, did you mean:
```

このaquaのアンインストールモードは環境変数で設定できます。
