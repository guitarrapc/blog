---
Title: dotnet testでテスト失敗時に自動的にリトライする
Category:
- C#
Date: 2025-01-31T13:08:09+09:00
...
今回は、次のような`dotnet test`をリトライできるようにしてみましょう。`RunConfiguration.TestSessionTimeout`[^2]を指定するなどいろいろやってます。
...
retry_limit=3
current=0
while ! dotnet test-rerun --deleteReports --logger trx --logger "console;verbosity=normal" --logger GitHubActions --inlineRunSettings RunConfiguration.TestSessionTimeout=1000; do
  # on failure
  echo -n "Failed. "
