...
Category:
- C#
Date: 2025-01-22T03:00:35+09:00
...
Bashでファイル検索するときにグロブ(Glob)を使うことがあります。アスタリスク`*`を使ったワイルドカードに限定しますがC#でもグロブしてみましょう。

[:contents]

# グロブ検索とは

[glob (programming) - Wikipedia](https://en.wikipedia.org/wiki/Glob_(programming))に詳細がありますが、グロブ検索とは、ディレクトリ内のファイルやフォルダ名に対してパターンマッチングを行い、一致する名前を持つファイルやフォルダを検索する方法です。特定の名前パターン（ワイルドカードなど）を指定して検索する際に便利な手法です。グロブ検索は古くからある方法ですが、C#では[`Directory.EnumerateFiles`](https://learn.microsoft.com/ja-jp/dotnet/api/system.io.directory.enumeratefiles?view=net-9.0)や[`Directory.GetFiles`](https://learn.microsoft.com/ja-jp/dotnet/api/system.io.directory.getfiles?view=net-9.0)を利用することで、`*`を使ったワイルドカード検索や再帰的なディレクトリ探索が可能です。そのため、C#においてはglobのような専用の仕組みを利用する機会はあまり多くないと感じます。
...
このライブラリはグロブパターンのうち、ワイルドカード`*`と`**`に対応し、`?`や`[...]`での範囲指定もサポートしていません。ただし、範囲指定はレンジ相当処理があるのでこれで対応できます。微妙に手触りが違うと使いにくいので、制約を受け入れられるならこのライブラリでよいかと考えています。サポートパターンは次の通りです。

![image](https://github.com/user-attachments/assets/2dd3ce82-18e5-46e1-ac07-4c754ad8904d)

さて、先のようにファイル検索だけをしたいなら、あまり難しいことを考慮しなくても次のようにかけます。`GetGlobRootAndInputPattern`はパターンからルートディレクトリと検索パターンを取得するヘルパーメソッドなので無視します。重要なのは、`Matcher.AddInclude()`には検索するパターン、`Execute()`には検索ルートディレクトリを指定することです。`Files`プロパティにはマッチしたファイル名が入っているのですが、`Directory.EnumerateFiles`のように扱えるようフルパスに変換しています。
...
public static class GlobSearch
{
    public static IEnumerable<string> GetFiles(string pattern)
    {
        ...
        var files = new Microsoft.Extensions.FileSystemGlobbing.Matcher()
          ...
          .Files
          .Select(x => Path.Combine(rootDirectory, x.Path)); // マッチしたファイル名しか返さないので、フルパスに変換
        return files;
    }
    ...
    private static (string rootDirectory, string includePattern) GetGlobRootAndInputPattern(string pattern)
    {
        ...
        {
            if (item.Contains('*')) // Microsoft.Extensions.FileSystemGlobbing は ? glob patternをサポートしない
                break;
            indexOfRoot++;
        }
        // グロブパターンじゃない
        ...
        var rootMarker = normalizedPattern[0] == '/' ? "/" : ""; // 非Windowsはルートディレクトリが空文字列になるので、その場合は/を追加
        var rootDirectory = rootMarker + Path.Combine(splitted[..indexOfRoot].ToArray()); // .NET9ならPath.Combine(splitted[..indexOfRoot]); でok
...
public static class GlobSearch
{
    ...
    private static (string rootDirectory, string includePattern) GetGlobRootAndInputPattern(string pattern)
    {
        ...
        // グロブパターンじゃない
        if (indexOfRoot == splitted.Length)
            return (Path.GetDirectoryName(normalizedPattern) ?? Directory.GetCurrentDirectory(), Path.GetFileName(pattern)); // ルートディレクトリのみの場合はそのまま返す
