...
別の例として、GitHubActionsの実行コンテキスト`${{ github }}`でとれる値を使いたいケースもあります。そんな時はGitHubコンテキストでほしい情報をクラス定義し、環境変数経由で実行コンテキストを取得、フィルターでデシリアライズしておけば各コマンドでGitHubの情報を使いやすくなります。

GitHub Actionsでコマンド実行時に環境変数`GITHUB_CONTEXT`にGitHubの実行コンテキストをJSONで渡して置きましょう。
