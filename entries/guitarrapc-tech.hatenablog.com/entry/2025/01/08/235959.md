...
次のようにメソッドの前後にログ出力をいれてもいいのですが、10、50、100コマンドなど大量にコマンドがあると面倒です、やってられない！
...
ConsoleAppFrameworkの`ConsoleAppFilter`を使うと、コマンド本体の前後に処理を挟むことができます。コマンド本体に処理が露出しないので、コマンド本体には本当にやりたいことだけを書いて、前後処理はフィルターに任せることができます。

詳しい使い方は[README](https://github.com/Cysharp/ConsoleAppFramework?tab=readme-ov-file#filtermiddleware-pipline--consoleappcontext)を見るとして、使い方を見ていきましょう。

**ConsoleAppFilterの基本的な使い方**

自前のフィルターを作るには、`ConsoleAppFilter(next)`を継承したクラスを作り`InvokeAsync`メソッドをオーバーライドします。[^3]
...
`Next.InvokeAsync`前後に任意の処理を書くことで、コマンド本体に到達する前、コマンド終了後に処理を差し込めます。
