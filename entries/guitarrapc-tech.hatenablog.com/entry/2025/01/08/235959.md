...
今回は、C#のCLIフレームワークである[ConsoleAppFrameworkv](https://github.com/Cysharp/ConsoleAppFramework)で処理実行前後に処理を挟む方法を紹介します。
...
- 実行時のコマンド引数を解析しメソッドパラメーターにマッピング
...
- SourceGeneratorを使うことでAOT環境での動作もサポート

## ConsoleAppFrameworkの基本的な使い方

紹介した機能がCLIを書くのにどう必要か見てみましょう。例えば`foobarというサブコマンドでnameという引数を持たせて、"Hello, {name}!"と表示させるコマンドを用意する`というコマンドはConsoleAppFrameworkを用意します。
...
Visual Studioから簡単に実行するため、launchSettings.jsonで定義を用意しておきます。それぞれの定義は、コマンドに渡したい引数を指定します。
...
同様にfoobarコマンドを実行してみましょう。launchSettings.jsonで`--name foo`を指定しています。
...
もちろんdotnet runコマンドで実行も簡単です。ビルド不要なので、CIで実行するときはこれがいいですね。
...
CLIが簡単に作れるのはわかりましたが、この調子でコマンドを作っていくとコマンド本体の実行前後に処理を挟む機能がほしくなります。[^2]
...
次のようにメソッドの前後にログ出力をいれてもいいのですが、10コマンド、50コマンド、100コマンドと、大量にコマンドがあると面倒なことこの上ありません。やってられない！
...
そんな時にConsoleAppFilterを用います。

## ConsoleAppFilterとは

ConsoleAppFrameworkには、処理を前後簡単に挟むための仕組み`ConsoleAppFilter`があります。これを使うと、コマンド本体の前後に処理を挟むことができます。

**ConsoleAppFilterの基本的な使い方**

詳しい使い方は[README](https://github.com/Cysharp/ConsoleAppFramework?tab=readme-ov-file#filtermiddleware-pipline--consoleappcontext)を見るとわかりますが、`ConsoleAppFilter(next)`を継承したクラスを作り、`InvokeAsync`メソッドをオーバーライドすることで実装します。[^3]

自分のフィルターを作る時、ポイントはが3点あります。

- internalクラスで作ること: ConsoleAppFilterはinternalクラスなのでpublicクラスでは作れません
- コンストラクターで`ConsoleAppFilter next`を受け取り`base(next)`を呼び出すこと: Primary Constructorで書くと楽
- 実行時は`Next.InvokeAsync`を呼び出すこと: Nextは継承元クラスに実装されている[^4]

コマンド本体の呼び出しである`Next.InvokeAsync`前後に任意の処理を書くことで、コマンド本体に到達する前、コマンド終了後に処理を差し込めます。
...
フィルターは、グローバル、クラス、メソッドの3つのレベルで登録できます。フィルターは多段設定でき、登録準に実行されます。

- グローバル: どのコマンド呼び出し時も必ず呼び出される
- クラス: そのクラスのコマンド呼び出し時に呼び出される
- メソッド: そのメソッドのコマンド呼び出し時に呼び出される

`グローバル > クラス > メソッド`の順に呼び出されます。
...
特定のコマンド専用の前後処理を書くならメソッド単位で指定できると便利ですね。一方、コマンドの呼び出しログを出力させるフィルターを書くならグローバルに登録するといいでしょう。

## ConsoleAppFilterを使って前後処理を挟む

先の例で、コマンド本体の前後にコマンド呼び出しログを出力するフィルターを作ってみましょう。どのコマンドでも表示してほしいのでグローバルに登録します。
...
フィルターはいろいろなパターンで使えます。私が便利だと思ったのはZxで呼び出すコマンドが正常に動作する前提条件を満たしているかチェックするフィルターです。例えば、[gh](https://github.com/cli/cli)コマンドをGitHub Actionsで使う時は`GH_REPO`と`GH_TOKEN`環境変数が必要です。これをチェックするフィルターを作成して、コマンド実行前にチェックするといいでしょう。
...
ほかにもGitHubActionsでZxを使うにあたり、GitHubコンテキスト`${{ github }}`をデシリアライズしておけば、各コマンドでGitHubの情報を使いやすくなります。
...
`--help`を呼び出しでもフィルターは呼び出されます。フィルターはコマンド本体の前後に処理を挟むため、`--help`でも呼び出されるのは仕様です。(ヘルプもヘルプというコマンド)
