...
Category:
- C#
Date: 2025-01-30T02:59:15+09:00
...
OSプラットフォームによって処理を分ける必要があるので、Windows、Linux、OSXで処理を分けます。次の環境で動作確認しています。
...
OSXは他OSと比べて考慮が必要になっています。OSXはsysctlでCPUモデルを呼び出すのが主であるため、プロセス実行を避けるlibc経由で`sysctl`を呼び出しています。
ただDllImportを素直に書くとOSX以外でビルドするとエラーになるため、OSXでのみビルドするようにcsprojで`DefineConstants`に`OX_OSX`を登録して実行環境がOSXか判別しています。
...
csprojで次のようにOSごとの条件付きコンパイルシンボルを登録することで、プリプロセッサでOS判別できます。
...
DllImportするなら仕込むことになりますが、ちょっといやですね。ただ、macOSやWindowsでプロセス呼び出しよりP/Invokeするなら必要ですね。
