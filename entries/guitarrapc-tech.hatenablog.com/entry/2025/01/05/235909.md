...
C# (.NET)プロジェクトで外部パッケージを管理する方法はいくつかありますが、Visual Studio 2022で導入されたCentral Package Managementはプロジェクトのパッケージ管理を簡単にする新しい方法です。特に複数のcsprojファイルを持つソリューションで、パッケージとバージョンを一元管理できます。

これまで、複数.csprojでNuGetパッケージ定義をCentral Package Managementに移行する際には、手作業での移行が必要でしたが、Visual Studio 2022の`.NET Upgrade Assistant`を使うとCentral Package Management移行をアシストしてくれるので紹介します。
