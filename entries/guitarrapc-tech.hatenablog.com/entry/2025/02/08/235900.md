...
tflintを実行するには、terraformの初期化`init`、tflintの実行が必要です。また、modulesを利用している側でtflintを実行してもmodulesの詳細をチェックしてくれないので、modulesの中に潜ってtflintを実行します。ということで、以下のようなララッパークリプト`tflint.sh`を作成すると便利です。[^2]
