...
.editorconfigはファイル拡張子やファイル名をターゲットとして指定するため、感覚としては.gitattributesに似ています。ということは注意点も似ているということです。例えば、`*.cs`はC#のソースコードを指定しますが、`*.cs`はC#のプロジェクトファイルではありません。このため、`*.csproj`や`*.props`も指定します。
