...
特に、[プロジェクトのコード規約はMicrosoftのC#コード規則](https://learn.microsoft.com/ja-jp/dotnet/csharp/fundamentals/coding-style/coding-conventions)とすることは多いでしょう。多人数が関わるプロジェクトでは、[editorconfigでコード規約を設定する](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/code-style-rule-options)と明解かつ個人のIDE/エディター設定に依存せず、IDEのフルサポートを受けられるのでオススメです。
