---
Title: (続) C#でCPUモデルを取得する
Category:
- C#
Date: 2025-11-18T01:30:00+09:00
URL: https://tech.guitarrapc.com/entry/2025/11/18/013000
EditURL: https://blog.hatena.ne.jp/guitarrapc_tech/guitarrapc-tech.hatenablog.com/atom/entry/17179246901317525962
---

[以前の記事](https://tech.guitarrapc.com/entry/2025/01/28/235900)でC#からCPUモデルを取得する方法を紹介しましたが、ARM64環境に対する手当が不十分でした。今回はARM64環境も含めてC#でCPUモデルを取得する方法を紹介します。
...
| x86_64 | Linux | `/proc/cpuinfo`を読む、lscpuを使う |
| x86_64 | Windows | WMI (Win32_Processor)を使う、レジストリを読む |
| x86_64 | macOS | sysctlを使う |
| ARM64 | 共通 | なし |
| ARM64 | Linux | lscpuを使う、`/proc/cpuinfo`を読んでマッピング参照 |
| ARM64 | Windows | WMI (Win32_Processor)を使う、レジストリを読む |
| ARM64 | macOS | sysctlを使う |

x86_64環境ではCPUID命令を使ってCPUモデルを直接取得できるのに対し、ARM64環境ではCPUID命令が存在しないため、OS依存の方法で取得する必要があります。具体的にはOSが提供するAPIやコマンドを利用してCPUモデルを取得しましょう。
特にLinux環境では、x86_64で`/proc/cpuinfo`にCPUモデル情報が含まれていたのがARM64では含まれていないため、`lscpu`コマンドを使うか`/proc/cpuinfo`を読んでマッピングテーブルを参照する必要があります。

## C#での実装例

今回は、x86_64とARM64環境の両方をサポートしています。

Linux ARM64環境でCPUモデルを取得するために`/proc/cpuinfo`を読んでARMのImplementer(CPUベンダー)とパーツ番号からモデル名をマッピングする方法を採用しました。
マッピング表は、[util-linux/util-linux | GitHub](https://github.com/util-linux/util-linux/blob/v2.41.2/sys-utils/lscpu-arm.c)と[fastfetch-cli/fastfetch | GitHub](https://github.com/fastfetch-cli/fastfetch/blob/2.55.1/src/detection/cpu/cpu_arm.h)を参考にしています。が、この実装ってCPUベンダーが新しいCPUを出すたびに更新しないといけないので、なかなか大変です。とはいえ、[この記事](https://blog.arthur1.dev/entry/2024/02/22/094044)でも述べられているように、[lscpu](https://kernel.googlesource.com/pub/scm/utils/util-linux/util-linux/+/refs/tags/v2.41.2/sys-utils/lscpu-arm.c)も内部実装はマッピングなわけで、ARM64環境でCPUモデルを取得するにはこの方法が2025年時点では最も確実です。[^1]
...
適当なProgram.csを用意して実行します。.NET 10ならProgram.csファイル1つあれば`dotnet Program.cs`で実行可能です。
...
出力結果です。全部の環境でCPUモデルが正しく取得できていることがわかります。GitHub ActionsのホストランナーはAzure VMなのですが、Ubuntu ARM64ではNeoverse-N2、Windows ARM64ではCobalt 100が使われているのが興味深いですね。[Azure Cobaltドキュメント](https://learn.microsoft.com/ja-jp/azure/virtual-machines/sizes/cobalt-overview)にあるように、Azure Cobalt 100はNeoverse-N2ベースのカスタムSoCなので、整合性が取れています。
...
前回はx86_64環境のみの対応でしたが、今回はARM64環境も含めてC#でCPUモデルを取得する方法を紹介しました。今回はマッピングを書きましたが、マッピングテーブルをメンテし続ける必要があるのは面倒なので`lscpu`コマンドを実行して省力もよいでしょう。実行頻度やパフォーマンス要件に応じて使い分けてください。

## 参考

- [C#でCPUモデルを取得する](https://tech.guitarrapc.com/entry/2025/01/28/235900)
- [arm64の場合/proc/cpuinfoでCPUのモデル名が取得できない件とその対策 - Diary of a Perpetual Student](https://blog.arthur1.dev/entry/2024/02/22/094044)
- [util-linux/util-linux - GitHub](https://github.com/util-linux/util-linux)
- [fastfetch-cli/fastfetch - GitHub](https://github.com/LinusDierheimer/fastfetch)
- [kernel/sys-util - lscpu.c | Google Source](https://kernel.googlesource.com/pub/scm/utils/util-linux/util-linux/+/refs/tags/v2.41.2/sys-utils/lscpu-arm.c)
- [Azure Cobalt プロセッサベースの仮想マシン - Microsoft Learn](https://learn.microsoft.com/ja-jp/azure/virtual-machines/sizes/cobalt-overview)
