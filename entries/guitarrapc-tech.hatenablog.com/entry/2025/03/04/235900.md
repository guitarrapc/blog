...
2024年12月にFargateがARM64 Spotに対応したこともあり、ECSをamd64で動かしていたがarm64にしたい。あるいは両アーキテクチャで動かしたい、というケースが現実を帯びるようになりました。こういった時、まずマルチプラットフォームなイメージに対応しておくことで、「Task Definitionのイメージタグ構成はかえず」に「ECS Service Providerをamd64/arm64で調整」するだけでスムーズなアーキテクチャ変更ができます。なるべく上流から対応することで柔軟な運用が可能になるため、マルチプラットフォームDockerイメージ作成とECRプッシュは重要です。
