...
.slnの特徴として、プロジェクトごとにGUIDが振られています。このGUIDはソリューションにプロジェクトを追加すると勝手にふられ、機会は少ないものの手動編集するには無駄に面倒でした。またファイルヘッダー部分でVisual Studioのバージョンも書かれており、最後に開いた人の環境が.slnとずれていると更新されていました。
...
結果として、.slnファイルはGit差分競合を起こしやすく、バージョン管理の面でも可読性の面でも問題がありました。
