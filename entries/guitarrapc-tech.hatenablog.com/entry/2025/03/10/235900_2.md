...
Category:
- .NET
Date: 2025-03-23T05:43:23+09:00
URL: https://tech.guitarrapc.com/entry/2025/03/10/235900_2
...
.NET SDK 9.0.200からSLNX形式と呼ばれる新しいソリューションファイルフォーマットが利用しやすくなりました。
...
拡張子`.sln`のソリューションファイルは次のように独自形式になっています。
...
せっかくcsprojがシンプルになったので同様にslnxもシンプルになってほしい、という[要望](https://github.com/dotnet/msbuild/issues/1730#issuecomment-281446717)は以前から出ており、2024年から満を持してSLNX形式がプレビュー提供されました。

# SLNX形式

SLNX形式は次のようなフォーマットです。csproj同様にXMLで定義され、割と人間が理解しやすい形式になっています。
...
SLNXソリューション形式は以下のような特徴でシンプルになっています。特にコメントを残すこともできるようになったことと、Git差分競合が起こりにくくなっていて実用的だと感じます。

| 要素 | .sln | .slnx | 個人的な所感 |
| --- | --- | --- | --- |
| ヘッダーセクション | あり | なし | 行数削減、環境非依存 |
| プロジェクト | GUIDで管理 | パスで管理 | プロジェクトの追加削除で競合が起こりにくい |
| プラットフォーム | `SolutionConfigurationPlatforms`で管理 | `<Configurations.Platform>`で管理 | シンプルな定義 |
| プロジェクトごとのプラットフォーム| GUID+プラットフォーム文字列で管理 | プロジェクトで共通なら指定なし | 行数削減 |
| コメントを入れられるか | 不可 | 空白、コメントが保持されるようになった | メンテナンス性向上 |
...
.slnxファイルは、Visual Studio Selectorで認識していない拡張子ですが、Visual Studio 2022以降[^2]で扱うことができます。(拡張子紐づけがはじめはないので、Visual Studio Selectorを選んでおきます)
...
また、.slnと.slnxの両方を同期するのは人間がやるには厳しいためどちらかに絞るか、両方併用するなら[dotnet-sln-sync](https://github.com/edvilme/dotnet-sln-sync)[^1]の利用が推奨のようです。
