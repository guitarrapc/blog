...
hatena/Hatena-Blog-Workflows-Boilerplateとhatena/hatenablog-workflowsは、共にReusable Workflowやアクションをタグ指定しています。昨今のSHA指定でないとまずいという流れを考えると微妙な設計です。ボイラーテンプレートのREADMEを見ると`workflowの変更は原則 Reusable workflows を変更するため基本的には更新は不要です`と書いてあり、v1タグを指定している意図が分かります。
...
私の運用では、ボイラーテンプレートをフォークして運用するのではなく、コピー&ペーストしてファイル名頭に`_`をつけて運用しています。自分のポリシーと会わないものだけをコピーして修正、あとはボイラーテンプレートのものを使うという運用です。ポリシーは次の通りです。
