...
ワークフローの静的解析をするツールは次の通りです。これだけだと指摘が多すぎると対応が困難になるため、ワークフローを自動修正するヘルパーツールも併用します。
...
セキュリティプラクティスをワークフローに自動適用するヘルパーツールは、静的解析で指摘されるいくつかを修正してくれます。残念ながらごく一部の修正のみですが、まずはこれを実行してから静的解析して修正するかどうか決めるといいでしょう。
...
ghalintには現在、pinactやdisable-checkout-persist-credentialsのような自動修正機能はありませんが、これらが追加されると今どきのlintツールっぽい気がします。例えば、`--fix`オプションをつけて構文を自動修正する機能があれば便利だと感じます。
