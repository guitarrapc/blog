...
URL: https://tech.guitarrapc.com/entry/2026/01/06/230000
...
暗号化したシークレットファイル`secrets.yaml`から値を取得し、SSM Parameter Storeに登録するTerraformリソースを定義します。SOPSプロバイダーにv1.3.0で追加されたエフェメラルリソース`ephemeral "sops_file"`を用いると、Stateファイルに値が保存されません。従来の`data "sops_file"`リソースはStateファイルに値が保存されてしまうため避けましょう。
