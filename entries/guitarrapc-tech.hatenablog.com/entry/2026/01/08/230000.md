---
Title: コンテナでGUIアプリケーションを動かしてホストからブラウザで操作する
Category:
- Container
Date: 2026-01-08T23:00:00+09:00
URL: https://tech.guitarrapc.com/entry/2026/01/08/230000
...
今回は、それを一歩進めてブラウザ経由でコンテナのGUIアプリケーションにアクセスする方法を紹介します。引き続き、[guitarrapc/docker-jmeter-gui](https://github.com/guitarrapc/docker-jmeter-gui)リポジトリのDockerfileを利用します。
...
[Dockerfile](https://github.com/guitarrapc/docker-jmeter-gui/blob/5.6.3/src/ubuntu24.04/Dockerfile)を示します。以下を考慮しています。
...
[kasmvnc.yaml](https://github.com/guitarrapc/docker-jmeter-gui/blob/5.6.3/src/ubuntu24.04/kasmvnc.yaml)は以下の通りです。コマンドライン引数で指定もできるのですが、設定ファイルでまとめておくと見通しがよく、利用時にオーバーライドしやすいのでいい感じです。
...
[Dockerfile](https://github.com/guitarrapc/docker-jmeter-gui/blob/5.6.3/src/alpine3.23/Dockerfile)を示します。以下を考慮しています。
...
[supervisord.conf](https://github.com/guitarrapc/docker-jmeter-gui/blob/5.6.3/src/alpine3.23/supervisord.conf)は以下の通りです。
