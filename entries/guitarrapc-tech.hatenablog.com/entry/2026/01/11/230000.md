...
プロジェクトごとに異なるパッケージを参照することもある[^2]ので挙動としては理解できますが、packages.lock.jsonの役割的にはDirectory.Packages.propsのパスに1つだけ生成される方が自然な気はします。ただ、`.csproj`でパッケージをオーバーライドする場合もあるので、今の設計のままになりそうです。
