...
例えば私の場合、あるワークフローでネイティブビルドを行っていて10分かかります。そのビルド成果物を別のワークフローで使い回したいときに、一度ビルドしたアーティファクトを他のワークフローでダウンロードして使いたいというケースでした。ビルドを何度も繰り返すのは無駄ですよね。
...
色々検索するとGitHub APIを駆使して取得する例もありますが、`actions/download-artifact`で簡単にできるのでおすすめです。
