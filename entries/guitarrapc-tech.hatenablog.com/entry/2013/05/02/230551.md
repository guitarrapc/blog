...
あ、AWSのRDSですが……時間がUTC +0:00です。
これは、本記事のクエリで日本時間の現在時刻 (UTC +9:00)を入れられるようになります。自分で入れる場合は、 jst_now関数をたたくイメージです。
...
今回の場合は、AWSがUTC0:00なため、日本時刻にするためには`+9:00`します。現在時刻から自動的に算出した値を返す関数を作っています。
...
今回の場合は、AWSがUTC 0:00なため、日本時刻にするためには +9:00します。
