...
[Finding Built-In Variables](http://powershell.com/cs/blogs/tips/archive/2013/02/20/finding-built-in-variables.aspx)
...
ただ、これでは、ビルトイン変数とユーザー定義変数がごっちゃ混ぜになってしまいます。`Get-Variable | select *`や`Get-ChildItem variable:`では区別がつかないのでしょうか？ つかないのです。
...
$other | ForEach-Object {$builtinVariable += $_}
```
...
```ps1
Get-Variable | Where {$_.Name -notin $builtinVariable} | select Name, Value, Description
```
