...
このパイプラインからの入力で、繰り返し処理を行うために利用するのが、`Begin{} Process{} End{}`です。
...
PowerShell 2.0において、`Job`の考えや手法が導入されましたが、これを利用して非同期処理を行うことは困難を極めます。なぜなら`Start-Job`や`-AsJob`が行っているのは、バックグラウンドに別の`Runspace`を1つ生成して、バックグラウンド処理を行っているだけです。
