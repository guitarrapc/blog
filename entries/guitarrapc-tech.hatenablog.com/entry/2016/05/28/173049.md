...
今回は、OSが入ったりしたディスク3を`消去 > 単一パーティション > NTFSフォーマット`まで構成してみましょう。
...
PowerShellには、ディスクやPartition、Volume操作用のCmdletがあります。これらをパイプラインで繋げれば、初期化からフォーマットまでワンライナーで綺麗に完了します。((PowerShellを管理者権限で起動します))
