...
中にはデフォルトから変更しないことをすすめる記事もあります。これはこれで有益です。特に ```ps1.exe -ExeuctionPolicy RemoteSigned -Command "なにか処理やps1"```は 私自身よく利用しています。
...
例えば、```ps1.exe -ExecutionPolicy RemoteSigned``` などは、Process スコープの変更です。
