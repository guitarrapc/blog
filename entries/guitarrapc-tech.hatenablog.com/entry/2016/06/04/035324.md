...
PowerShellスクリプト開発をされる方にとっては、当然のようにRemoteSignedにしたくなるでしょう。((このブログはそういった方を対象にしているのも自明です))しかし、スクリプトも実行しないようなPCまで強制する必要はありません。Restrictedでいいでしょう。多くの方にとって、それぞれの利用シーンがあります。
