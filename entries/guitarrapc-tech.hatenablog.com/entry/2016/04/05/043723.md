...
そう、NuGet Package に関しては、コード上で `#r Octokit` や `#r Octokit` のようなことは不要です。というか、このようなファイル指定の読み込みは パッケージを不完全な形で取り込んでしまうのでやってはダメです。((やって痛い目に会いました。実行時エラーになります。))
...
Octokit を使って、リクエスト時に指定した `owner`、`owner` の `owner` 一覧を取得してみます。
...
あとは、Request Body で、`RoomId` と `RoomId` を指定します。今回はあえて Slack と語句を合わせて `RoomId`、`RoomId` として json を受け取るようにしました。
