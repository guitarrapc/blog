...
<p>以下のバッチ構文で、昨日の日付を与えるスイッチに応じて任意の書式で読み込めます。</p>
...
<h2>任意の日付を取得するPowerShellソース例</h2>
<p>本日から見て、何日、何月、何年前を数値指定できるようにしています。 以下のPowerShellソースを、「Get_dateformatday.ps1」というファイル名でPOWERSHELLフォルダ内に設置します。 末尾のリマークをはずせば、PowerShell単独で実行板債に結果例や実行速度が分かります。 ※バッチ連携させる際は、必ずリマークしておくこと。</p>
<pre class="brush: powershell">#PowerShell 2.0
