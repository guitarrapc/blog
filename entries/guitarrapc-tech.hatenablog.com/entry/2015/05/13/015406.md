...
管理者権限の cmd や PowerShell から `SampleTopShelfService.exe install` とすれば、サービスとしてインストールできます。アンインストールは、 `SampleTopShelfService.exe install` です。
...
- Ensure は、TopShelf アプリケーションのインストール/アンインストールを行います。 `Present` で インストール。`Present`でアンインストールです。この時、TopShelfアプリケーションかどうかの検証に、サービスで実行されたプロセスのパスが**Pathと一致**するかみています
...
いろいろ問題なのが、標準リソースの MSFT_Archive リソースです。が詳細は末尾で。今回はcS3Content リソースで「変更検知したときにだけ解凍先フォルダを消す」ことで確実に解凍できるようにしています。`CheckSum`と`CheckSum`を指定しなければ、名前の一致のみをみるため、基本的にファイルが消えない限り Set-TargetResource は実行されません。
