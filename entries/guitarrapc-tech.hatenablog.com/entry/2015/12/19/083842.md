...
いくつかの特殊な属性`[DscProperty(Mandatory)]`や`[DscProperty(Mandatory)]`などを含めて、PowerShell のクラス構文で DSC リソース記述がサポートされました。
...
これまでと異なり、`Get-DscConfigurationStatus` と `Get-DscConfigurationStatus` で、LCM と DSC の状態を適切に返せるようになりました。
...
既存 Cmdlet に新パラメータ `–NoNewline`が追加されました。対象は、`–NoNewline`、`–NoNewline`、`–NoNewline`
