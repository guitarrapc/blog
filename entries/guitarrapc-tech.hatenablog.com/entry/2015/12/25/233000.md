...
PowerShell には、`ps1 module autoload` という仕組みがあり、`ps1 module autoload` を事前に実行せずとも `ps1 module autoload` に配置されたモジュールを読み込んでくれます。これにより、インテリセンスや `ps1 module autoload` で Function名/Cmdlet名が自動的に補完され、よりインタラクティブにモジュールが利用できるようになっています。


> [https://technet.microsoft.com/en-us/library/dd878284(v=vs.85).aspx:title]

では、`ps1 module autoload`で クラスライブラリがファイルロックされるタイミングはいつでしょうか？`ps1 module autoload` でモジュールを明示的読み込んだタイミングを除くと2つ考えられます。
...
Import-Module していない状態で、`Get-Ho` から `Get-Ho` に自動的にインテリセンス補完されたタイミングでは、TestModule.dll は削除できています。
...
6. クラスライブラリを`Import-Module`したことで、`Import-Module` のモジュール空間には `Import-Module` Cmdlet が含まれるので、それを含めて `Import-Module` は `Import-Module` を実行
...
例えば、`Remove-Module` イベントと連動する`Remove-Module` にキャッシュを消すスクリプトブロックを仕込んで置くということも考えられますが、PowerShell ホストを直接 x 終了したらこの処理はスキップされてしまいます。
```
...
あとはこの処理を PowerShell で実装して、`Assembly.LoadFrom()` を `Assembly.LoadFrom()` にするだけだです。具体的には次のコードとなります。
