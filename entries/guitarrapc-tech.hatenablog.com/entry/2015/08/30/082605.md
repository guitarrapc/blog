...
過去、PoweShell Teamブログで`using`による`try{}finally{}`が紹介されたこともあります。
...
usingときくと、C# では名前空間の宣言やリソース破棄がパッと思い浮かびます。PowerShellでも何の機能を持たせるか議論があったのですが、搭載されたのは名前空間の宣言です。
...
例えば`List<int>`がPowerShell 4.0までこう書く必要がありました。
...
アンドキュメントとはいえ、Windows 10で搭載された以上は消えないでしょうから使っちゃいましょうかね。
