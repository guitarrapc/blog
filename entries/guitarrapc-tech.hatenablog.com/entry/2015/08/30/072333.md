...
Windows 8.1にWMF5.0を当ててもPSRealtineが有効にならない場合、Windows 10にあげられると幸せになれるでしょう。
...
<details><summary>Windows 10 は RTM?</summary>
...
- WMF 5.0 RTMリリース後も3カ月間フルサポートされる
- PPの機能はすべてProduction Ready。つまり本番で利用可能。Experimentalな実装は含まない
- PPインストール前に、これまでのWMFプレビューリリースはアンインストールが必要(Windows 10はそのままあげていい)

「フルサポート」とは、本番環境を妨げる原因を調査しワークアラウンドを提供するという意味のようです。
