...
さらに、AD環境に置いては通常はローカルマシンの管理者権限 (Administrators) を許可していないことも多いです。(`Set-ExecutionPolicy`はAdministrators権限が必要です)
...
ExecutionPolicyにnullを突っ込む手法ですね。管理者権限もいらないので、この手法が許されるならいいです。((もう一個抜け道がありますがここでは触れません))
