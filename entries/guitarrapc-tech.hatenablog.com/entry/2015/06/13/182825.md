...
Microsoft.AspNet.WebPagesは、RazorEngineを今回ViewEngineに使うのですが、インテリセンスのために`bin/`直下へ置く必要がありました。
...
View/Apiも用意したら、デバッグ実行してみてください。 `https://localhost:8080/pages/Views/Index`にアクセスして、うまくViewが表示されればokですね！
