...
どういうことかというと、`git-credential-winstore.exe` や`git-credential-winstore.exe` が主に利用されるでしょうが、どっちを使っても GitHub認証情報が吹き飛びます。主に、gitプロセスが壊れたりすると認証情報も吹っ飛ぶます。
...
本モジュールでは、`git` という名称で別に Windows認証マネージャーに認証情報を設定しています。これにより、`git`がもし吹き飛んでいた場合は自動補完してくれます。
