...
1. 認証処理をクリアするため`valentia`を用いているため、[valentia](https://github.com/guitarrapc/valentia)のインストールが必要
2. `git`をインストール
...
- コードでは、前提のvalentiaインストールを含む
- コードでは、前提のGitContinousPullインストールを含む
