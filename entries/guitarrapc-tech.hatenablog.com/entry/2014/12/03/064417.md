...
1日目、2日目を通して、DSCやCMツールがなぜ必要なのかその意義について説明しました。頑張らなくてもできるけど、めんどくさいことを任せられるっていうのはIaaS -> PaaS -> SaaSにつながる大事な感覚です。

さて、3日目の今日は、PowerShell DSCについて見ていく前に、ChefとPowerShell DSCの経緯、両者の比較しChef/DSCを比べながら用語の整理をしましょう。
PowerShell DSCがなんとなくしか想像できなくても、Chefをイメージしやすいので少しイメージを膨らませます。

[:contents]

# Chef と PowerShell

PowerShell DSCを知らなくても、ChefチームとPowerShellチームの蜜月関係は耳にしたことがある方もいるでしょう。ChefがDSCを使うことについて、PowerShellチームのトップである[Jeffrey Snover](https://twitter.com/jsnover)の言葉を、[Chef Brings DevOps Platform and Practices to Windows and Microsoft Azure - Chef Blog](https://www.chef.io/blog/2014/09/08/chef-brings-devops-platform-and-practices-to-windows-and-microsoft-azure/:title)から引用しましょう。
...
1. これまで通り必要なクックブック(DSC Cookbooks) を使う
2. レシピを書く
3. 適用する
...
Windowsを管理する人にとってPowerShellは背中を押ししてくれます。Microsoftも期待に応えて、OS/アプリケーションの自動化をPowerShellに移行/公開してきました。
...
- 新規Chefユーザーにとってもいい話
- 既存のChefパターンもScriptリソースで実施できる
- OSの低レイヤへのアクセスも、Cや静的言語を介することなく可能
...
| 2013年 | Native PowerShellスクリプトリソースが公開されました。32/64bitを制御でき、Chefでは bash や他のスクリプトリソースと同等になりました。 |
...
記事を読んでくださっている方で「WindowsでPowerShellにて全部構成しよう」と思った方は、Chefが直面したのと似たような問題もあります。決して他人事ではありません。
...
- 「どうやるか」の手順ではなく、「どのような状態」でいてほしいかを`Declarative Syntax`((宣言的記法)) で簡潔に表現
...
ChefとDSCを比較しながら用語をまとめていきましょう。Chefを利用したことがある人にとってはDSCがより身近です。DSCを使っている人にとっては、Chefが身近に感じられるでしょう。
...
DSCで構成をするということは、コードで構成するということです。つまり、アプリケーションと同様にインフラがコードになります。

- バージョン管理システムでコード管理ができる
- コードから自動的なテスト、展開/配置、適用までアプリケーションと同様に扱える
- コードがバージョン管理をされると、インフラもバージョン管理できる
- 職人のマウスさばき(謎)もなく、コードという形で知識が共有される

設定をコードで行うことは、慣れてないと難しいと感じませんが、それに勝るメリットがあります。
...
「あるべき状態を宣言」する構文になっているため、よく **宣言型構文(Declarative Syntax)**と呼ばれます。((わかり難い人は、DSC用の何をしたいかわかりやすくした構文があると思っていただけるといい))
...
分離によって、利用者(呼びだす時)はリソースを準備するだけで再利用できます。((OSSで公開されたものでも、自分で書いてもいいです))
...
DSCやChefも、あるべき状態を維持します。一度設定しても、状態が変わったことを検知して再度あるべき状態にする。

この、あるべき状態を維持する機能をDSCやChefももっています。((DSCはPullに限ります。))

この機能がないとどうなるでしょうか？ タスクスケジューラ/cronで繰り返し実行をスケジュールするようなカオスな世界になります。

放っておいても、自動的にあるべき状態へ収束するのは大事です。
...
ChefやDSCも、設定する側と設定される側がいます。

クライアント/エージェントツールは、設定される側で「どうやってあるべき状態の設定を受け取ったり、取得したりするのか」、まさに **エンジン** ととも言うべき役割を果たします。

- Chef : chef-clientがクライアント/エージェントツールに該当
- DSC : Local Configuration Manager(以下LCM) が該当
...
- DSCのPUSHモードは、PSRemotingを介してノードに接続する
- DSCがLinuxにPUSHで構成できるのは、このWSMan (PSRemoting) を使って、LinuxにインストールしたOMIへ接続できるため
...
Chefは、DSCを利用できるといいましたが、コンフィグレーションだけでなかう、コンパイルして生成されたMOFを直接読んでDSCに適用できます。
