...
`Get-DscLocalConfigurationManager`をノードで実行した時に表示された、LCMのプロパティに`RefreshMode`があります。ここで現在のモードが確認できます。
在の流れでは、`RefreshMode`はPUSHになっているので、これを`PULL`に切り変えましょう。
...
15分に一度DSCサーバーへ構成の更新がないか確認し、30分に1回自身の構成を確認します。
...
この時、`ConfigurationMode`が`ConfigurationMode`になっているので、ノード自身の構成がDSCサーバーのあるべき状態と違っていたら自動的にあるべき状態へ収束します。
